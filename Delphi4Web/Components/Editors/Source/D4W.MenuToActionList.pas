...
type
  TD4WMenuToActionList = class(TActionList)
  private
    FMainMenu: TMainMenu;
    FExecute: Boolean;
...
procedure TD4WMenuToActionList.Notification(AComponent: TComponent;
...
procedure TD4WMenuToActionList.SetExecute(const Value: Boolean);
begin
  if Value then
    BuildActionList;
end;

procedure TD4WMenuToActionList.SetMainMenu(const Value: TMainMenu);
begin
  FMainMenu := Value;
end;

procedure TD4WMenuToActionList.BuildActionList;
var
  c: integer;
  ...
  methodType.Invoke(method, [mapModel { Sender } , nil { Area } ]);
    ...
    *)
    ...
    begin
      ...
      // cannot start with a number, and we don't want a begining _
