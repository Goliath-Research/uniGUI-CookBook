...
type
  ...
  // (rom) bit value to check IsConnectionDef of THIDPLinkCollectionNode
const
  lcnIsAlias = 1;
...
// tree.
  ...
  HIDP_LINK_COLLECTION_NODE = record
    ...
    IsConnectionDef:          BYTE;    // This link node is an alias of the next link node.
...
// collections, listed in the link collection node array, have their IsConnectionDef
...
// collection array once for each time they see the IsConnectionDef flag set, and use
// the last link collection as the index for the below accessor functions.
//
// NB: if IsConnectionDef is set, then NextSibling should be one more than the current
