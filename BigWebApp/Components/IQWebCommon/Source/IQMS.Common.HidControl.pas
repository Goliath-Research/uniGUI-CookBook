...
uses
  ...
  Vcl.Forms,
  {$ENDIF COMPILER6_UP}
  IQMS.Common.DBT, IQMS.Common.HIDSetupApi, IQMS.Common.HID, IQMS.Common.ModuleLoader, Vcl.Dialogs;
...
type
  ...
  TWebIQHidDeviceController = class;
  THidDevice = class;

  // the Event function declarations
  THidEnumerateEvent = function(HidDev: THidDevice;
    const Idx: Integer): Boolean of object;
  THidPlugEvent = procedure(HidDev: THidDevice) of object;
  THidUnplugEvent = THidPlugEvent;
  THidDataEvent = procedure(HidDev: THidDevice; ReportID: Byte;
    const Data: Pointer; Size: Word) of object;
  THidDataErrorEvent = procedure(HidDev: THidDevice; Error: DWORD) of object;

  // check out test function
  THidCheckCallback = function(HidDev: THidDevice): Boolean; stdcall;

  // open overlapped read or write file handle
  THidOpenExMode = (omhRead, omhWrite);

  // the physical descriptor
  TJvPhysicalDescriptor = array of WORD;

  // all USB relevant driver entries in the registry
  THidPnPInfo = class(TObject)
  private
    FDeviceID: DWORD;
    FDevicePath: string;
    FCapabilities: DWORD;
  ...
  public
    ...
    constructor Create(APnPHandle: HDEVINFO; ADevData: TSPDevInfoData; ADevicePath: PAnsiChar);
    destructor Destroy; override;
  end;

  // a thread helper class to implement THidDevice.OnData

  THidDeviceReadThread = class(TThread)
  private
    FErr: DWORD;
    procedure DoData;
    procedure DoDataError;
    constructor CtlCreate(const Dev: THidDevice);
  public
    Device: THidDevice;
  ...
  THidDevice = class(TObject)
  private
    // internal control variables
    FMyController: TWebIQHidDeviceController;
    ...
    FUsageParam: TUsage;
    FData: THidDataEvent;
    FDataError: THidDataErrorEvent;
    FUnplug: THidUnplugEvent;
    FHasReadWriteAccess: Boolean;
    FDataThread: THidDeviceReadThread;
    FTag: Integer;
    ...
    procedure SetDataEvent(const DataEvent: THidDataEvent);
    ...
    // Constructor is hidden! Only a THidDeviceController can create a THidDevice object.
    constructor CtlCreate(const APnPInfo: THidPnPInfo;
      const Controller: TWebIQHidDeviceController);
  protected
  ...
  public
    ...
    destructor Destroy; override;
    // methods
    function CancelIO(const Mode: THidOpenExMode): Boolean;
    procedure CloseFile;
    procedure CloseFileEx(const Mode: THidOpenExMode);
    ...
    function OpenFileEx(Mode: THidOpenExMode): Boolean;
    ...
    property PnPInfo: THidPnPInfo read FPnPInfo;
    ...
    // event properties
    property OnData: THidDataEvent read FData write SetDataEvent;
    property OnDataError: THidDataErrorEvent read FDataError write FDataError;
    property OnUnplug: THidUnplugEvent read FUnplug write FUnplug;
  end;

  // controller class to manage all HID devices

  {$IFDEF USEJVCL}
  THidDeviceController = class(TJvComponent)
  {$ELSE}
  TWebIQHidDeviceController = class(TComponent)
  {$ENDIF USEJVCL}
  private
    // internal properties part
    FHidGuid: TGUID;
    FArrivalEvent: THidPlugEvent;
    FDeviceChangeEvent: TNotifyEvent;
    FEnumerateEvent: THidEnumerateEvent;
    FDevDataEvent: THidDataEvent;
    FDevDataErrorEvent: THidDataErrorEvent;
    FDevUnplugEvent: THidUnplugEvent;
    FRemovalEvent: THidUnplugEvent;
    ...
    function CheckThisOut(var HidDev: THidDevice; Idx: Integer; Check: Boolean): Boolean;
    ...
    procedure SetEnumerate(const Enumerator: THidEnumerateEvent);
    procedure SetDevThreadSleepTime(const DevTime: Integer);
    procedure SetDevData(const DataEvent: THidDataEvent);
    procedure SetDevDataError(const DataErrorEvent: THidDataErrorEvent);
    procedure SetDevUnplug(const Unplugger: THidUnplugEvent);
  protected
    procedure DoArrival(HidDev: THidDevice);
    procedure DoRemoval(HidDev: THidDevice);
    procedure DoDeviceChange;
    function DoEnumerate(HidDev: THidDevice; Idx: Integer): Boolean;
  public
    // normal constructor/destructor
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    // methods to hand out HID device objects
    procedure CheckIn(var HidDev: THidDevice);
    function CheckOut(var HidDev: THidDevice): Boolean;
    function CheckOutByClass(var HidDev: THidDevice; const ClassName: string): Boolean;
    function CheckOutByID(var HidDev: THidDevice; const Vid, Pid: Integer): Boolean;
    function CheckOutByIndex(var HidDev: THidDevice; const Idx: Integer): Boolean;
    function CheckOutByProductName(var HidDev: THidDevice; const ProductName: WideString): Boolean;
    function CheckOutByVendorName(var HidDev: THidDevice; const VendorName: WideString): Boolean;
    function CheckOutByCallback(var HidDev: THidDevice; Check: THidCheckCallback): Boolean;
    ...
    function CountByCallback(Check: THidCheckCallback): Integer;
  ...
  published
    ...
    property OnArrival: THidPlugEvent read FArrivalEvent write FArrivalEvent;
    // the iterator event
    property OnEnumerate: THidEnumerateEvent read FEnumerateEvent write SetEnumerate;
    ...
    // these events are copied to THidDevices on creation
    property OnDeviceData: THidDataEvent read FDevDataEvent write SetDevData;
    property OnDeviceDataError: THidDataErrorEvent read FDevDataErrorEvent write SetDevDataError;
    property OnDeviceUnplug: THidUnplugEvent read FDevUnplugEvent write SetDevUnplug;
    property OnRemoval: THidUnplugEvent read FRemovalEvent write FRemovalEvent;
...
resourcestring
  ...
  RsEDirectHidDeviceCreationNotAllowed = 'Direct creation of a THidDevice object is not allowed';
...
//=== { THidDeviceReadThread } =============================================

constructor THidDeviceReadThread.CtlCreate(const Dev: THidDevice);
begin
...
constructor THidDeviceReadThread.Create(CreateSuspended: Boolean);
begin
  raise EControllerError.CreateRes(@RsEDirectThreadCreationNotAllowed);
end;

procedure THidDeviceReadThread.DoData;
begin
  Device.OnData(Device, Report[0], @Report[1], NumBytesRead - 1);
end;

procedure THidDeviceReadThread.DoDataError;
begin
...
procedure THidDeviceReadThread.Execute;
var
  SleepRet: DWORD;
begin
...
constructor THidPnPInfo.Create(APnPHandle: HDEVINFO; ADevData: TSPDevInfoData; ADevicePath: PAnsiChar);
begin
  inherited Create;
  FDeviceID := ADevData.DevInst;
  FDevicePath := string(ADevicePath);
...
destructor THidPnPInfo.Destroy;
begin
  FCompatibleIDs.Free;
  FHardwareID.Free;
  FLowerFilters.Free;
...
function THidPnPInfo.GetCompatibleIDs: TStrings;
begin
  Result := FCompatibleIDs;
end;

function THidPnPInfo.GetHardwareID: TStrings;
begin
  Result := FHardwareID;
end;

function THidPnPInfo.GetLowerFilters: TStrings;
begin
  Result := FLowerFilters;
end;

function THidPnPInfo.GetUpperFilters: TStrings;
begin
...
function THidPnPInfo.GetRegistryPropertyString(PnPHandle: HDEVINFO;
  const DevData: TSPDevInfoData; Prop: DWORD): string;
var
  BytesReturned: DWORD;
  RegDataType: DWORD;
...
function THidPnPInfo.GetRegistryPropertyStringList(PnPHandle: HDEVINFO;
  const DevData: TSPDevInfoData; Prop: DWORD): TStringList;
var
  BytesReturned: DWORD;
  RegDataType: DWORD;
...
function THidPnPInfo.GetRegistryPropertyDWord(PnPHandle: HDEVINFO;
  const DevData: TSPDevInfoData; Prop: DWORD): DWORD;
var
  BytesReturned: DWORD;
  RegDataType: DWORD;
...
// the constructor is only accessible from THidController
...
constructor THidDevice.CtlCreate(const APnPInfo: THidPnPInfo; const Controller: TWebIQHidDeviceController);
begin
  inherited Create;
...
// If a THidDevice is destroyed the THidController has to be informed.
// If the device is plugged in this THidDevice instance is destroyed,
// but another instance is created in the controller list to replace it.

destructor THidDevice.Destroy;
var
  I: Integer;
  TmpOnData: THidDataEvent;
  TmpOnUnplug: THidUnplugEvent;
  Dev: THidDevice;
begin
    ...
    begin
      ...
      for I := 0 to FList.Count - 1 do
        ...
        begin
          ...
          if IsPluggedIn then
          begin
            Dev := THidDevice.CtlCreate(FPnPInfo, FMyController);
...
function THidDevice.IsAccessible: Boolean;
begin
...
function THidDevice.OpenFile: Boolean;
begin
...
function THidDevice.OpenFileEx(Mode: THidOpenExMode): Boolean;
begin
  Result := False;
  // check if open allowed (propagates this state)
  if IsAccessible then
...
function THidDevice.GetDeviceStringAnsi(Idx: Byte): string;
begin
  Result := WideCharToString(PWideChar(GetDeviceStringUnicode(Idx)));
end;

function THidDevice.GetDeviceStringUnicode(Idx: Byte): WideString;
var
...
function THidDevice.GetLinkCollectionNode(Idx: WORD): THIDPLinkCollectionNode;
var
  Siz: ULONG;
begin
...
procedure THidDevice.SetNumInputBuffers(const Num: Integer);
begin
...
procedure THidDevice.SetNumOverlappedBuffers(const Num: Integer);
begin
...
procedure THidDevice.SetReportTypeParam(const ReportType: THIDPReportType);
begin
  FReportTypeParam := ReportType;
  GetMax;
end;

procedure THidDevice.SetThreadSleepTime(const SleepTime: Integer);
begin
...
procedure THidDevice.SetUsagePageParam(const UsagePage: TUsage);
begin
  FUsagePageParam := UsagePage;
  GetMax;
end;

function THidDevice.GetConfiguration: THIDDConfiguration;
begin
  Result.cookie := nil;
  Result.size := 0;
...
function THidDevice.GetPreparsedData: PHIDPPreparsedData;
begin
  if FPreparsedData = nil then
    if OpenFile then
    begin
...
function THidDevice.GetCaps: THIDPCaps;
begin
  FillChar(Result, SizeOf(THIDPCaps), #0);
  HidP_GetCaps(PreparsedData, Result);
end;

function THidDevice.GetVendorName: WideString;
var
  Buffer: array [0..253] of WideChar;
begin
  if FVendorName = '' then
...
function THidDevice.GetProductName: WideString;
var
  Buffer: array [0..253] of WideChar;
begin
...
function THidDevice.GetSerialNumber: WideString;
var
  I: Integer;
  Len: Integer;
...
function THidDevice.GetPhysicalDescriptor: TJvPhysicalDescriptor;
var
  I: Integer;
begin
...
function THidDevice.GetLanguageStrings: TStrings;
var
  I: Integer;
  Len: Integer;
  IDs: array [0..253] of WORD;
...
function THidDevice.GetOverlappedReadResult: DWORD;
begin
  Result := 0;
...
function THidDevice.GetOverlappedWriteResult: DWORD;
begin
  Result := 0;
...
procedure THidDevice.SetConfiguration(const Config: THIDDConfiguration);
begin
  if OpenFile then
    HidD_SetConfiguration(HidFileHandle, Config, SizeOf(THIDDConfiguration));
end;

procedure THidDevice.SetDataEvent(const DataEvent: THidDataEvent);
begin
...
function THidDevice.CancelIO(const Mode: THidOpenExMode): Boolean;
...
procedure THidDevice.CloseFile;
begin
  if HidFileHandle <> INVALID_HANDLE_VALUE then
...
procedure THidDevice.CloseFileEx(const Mode: THidOpenExMode);
begin
  if Mode = omhRead then
  begin
    if HidOverlappedRead <> INVALID_HANDLE_VALUE then
...
function THidDevice.FlushQueue: Boolean;
begin
  Result := False;
  if OpenFile then
...
function THidDevice.GetFeature(var Report; const Size: Integer): Boolean;
begin
  Result := False;
  if OpenFile then
...
function THidDevice.SetFeature(var Report; const Size: Integer): Boolean;
begin
  Result := False;
  if OpenFile then
...
function THidDevice.GetSpecificButtonCaps(ButtonCaps: PHIDPButtonCaps; var Count: WORD): NTSTATUS;
begin
...
function THidDevice.GetButtonCaps(ButtonCaps: PHIDPButtonCaps; var Count: WORD): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetSpecificValueCaps(ValueCaps: PHIDPValueCaps; var Count: WORD): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetValueCaps(ValueCaps: PHIDPValueCaps; var Count: WORD): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetData(DataList: PHIDPData; var DataLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.SetData(DataList: PHIDPData; var DataLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetUsages(UsageList: PUsage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetButtons(UsageList: PUsage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetUsagesEx(UsageList: PUsageAndPage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetButtonsEx(UsageList: PUsageAndPage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.SetUsages(UsageList: PUsage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.SetButtons(UsageList: PUsage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.UnsetUsages(UsageList: PUsage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.UnsetButtons(UsageList: PUsage; var UsageLength: ULONG;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetUsageValue(var UsageValue: ULONG; var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetScaledUsageValue(var UsageValue: Integer;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.GetUsageValueArray(UsageValue: PChar;
...
function THidDevice.SetUsageValue(UsageValue: ULONG; var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.SetScaledUsageValue(UsageValue: Integer; var Report;
...
function THidDevice.SetUsageValueArray(UsageValue: PChar;
  UsageValueByteLength: WORD; var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function THidDevice.DeviceIoControl(IoControlCode: DWORD; InBuffer: Pointer; InSize: DWORD;
  OutBuffer: Pointer; OutSize: DWORD; var BytesReturned: DWORD): Boolean;
begin
  Result := False;
  if OpenFile then
    Result := Winapi.Windows.DeviceIoControl(HidFileHandle, IoControlCode, InBuffer, InSize,
      OutBuffer, OutSize, BytesReturned, nil);
end;

function THidDevice.ReadFile(var Report; ToRead: DWORD; var BytesRead: DWORD): Boolean;
begin
  Result := False;
  if OpenFile then
    Result := Winapi.Windows.ReadFile(HidFileHandle, Report, ToRead, BytesRead, nil);
end;

function THidDevice.WriteFile(var Report; ToWrite: DWORD; var BytesWritten: DWORD): Boolean;
begin
  Result := False;
  if OpenFile then
    Result := Winapi.Windows.WriteFile(HidFileHandle, Report, ToWrite, BytesWritten, nil);
end;
...
function THidDevice.ReadFileEx(var Report; ToRead: DWORD;
  CallBack: TPROverlappedCompletionRoutine): Boolean;
begin
  Result := False;
  ...
  begin
    ...
    Result := IQMS.Common.HidControl.ReadFileEx(HidOverlappedRead, Report, ToRead, FOvlRead, CallBack);
  end;
end;

function THidDevice.WriteFileEx(var Report; ToWrite: DWORD;
  CallBack: TPROverlappedCompletionRoutine): Boolean;
begin
  Result := False;
  ...
  begin
    ...
    Result := IQMS.Common.HidControl.WriteFileEx(HidOverlappedWrite, Report, ToWrite, FOvlWrite, CallBack);
  end;
end;

function THidDevice.CheckOut: Boolean;
begin
...
function  THidDevice.GetExtendedAttributes(ReportType: THIDPReportType; DataIndex: Word;
  Attributes: PHIDPExtendedAttributes; var LengthAttributes: ULONG): NTSTATUS;
begin
...
function THidDevice.InitializeReportForID(ReportType: THIDPReportType; ReportID: Byte;
  var Report; ReportLength: ULONG): NTSTATUS;
begin
...
function THidDevice.GetInputReport(var Report; const Size: ULONG): Boolean;
begin
  Result := False;
...
function THidDevice.SetOutputReport(var Report; const Size: ULONG): Boolean;
begin
  Result := False;
...
//=== { TTWebIQHidDeviceController } =============================================

constructor TWebIQHidDeviceController.Create(AOwner: TComponent);
const
...
// unplug or kill all controlled THidDevices on controller destruction

destructor TWebIQHidDeviceController.Destroy;
var
  I: Integer;
  HidDev: THidDevice;
begin
  ...
  begin
    ...
    begin
      ...
      FMyController := nil;
      if IsCheckedOut then
        DoUnplug // pull the plug for checked out THidDevices
      else
        Free; // kill THidDevices which are not checked out
    end;
  end;
  FList.Free;
...
procedure TWebIQHidDeviceController.DoArrival(HidDev: THidDevice);
begin
...
procedure TWebIQHidDeviceController.DoRemoval(HidDev: THidDevice);
begin
...
procedure TWebIQHidDeviceController.DoDeviceChange;
begin
...
procedure TWebIQHidDeviceController.EventPipe(var Msg: TMessage);
begin
...
procedure TWebIQHidDeviceController.DeviceChange;
var
  I: Integer;
  J: Integer;
  HidDev: THidDevice;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            PnPInfo := THidPnPInfo.Create(PnPHandle, DevData, PAnsiChar(@FunctionClassDeviceData.DevicePath));
            // create HID device object and add it to the device list
            try
              HidDev := THidDevice.CtlCreate(PnPInfo, Self);
              NewList.Add(HidDev);
            except
...
begin
  ...
  begin
    ...
    for J := NewList.Count - 1 downto 0 do
      if (THidDevice(NewList.Items[J]).PnPInfo.DeviceID = HidDev.PnPInfo.DeviceID) and
  ...
  for I := 0 to NewList.Count - 1 do
    for J := 0 to FList.Count - 1 do
      if (THidDevice(NewList[I]).PnPInfo.DeviceID = THidDevice(FList[J]).PnPInfo.DeviceID) and
        THidDevice(FList[J]).IsPluggedIn then
      begin
        THidDevice(NewList[I]).FMyController := nil; // prevent Free/Destroy from accessing this controller
        THidDevice(NewList[I]).Free;
        NewList[I] := nil;
        Break;
      end;
  ...
  for I := 0 to NewList.Count - 1 do
    ...
    begin
      FList.Add(NewList[I]);
      Changed := True;
      DoArrival(THidDevice(NewList[I]));
    end;
...
class function TWebIQHidDeviceController.HidVersion: string;
var
  Dummy: DWORD;
  Size: UINT;
  Buf: string;
...
procedure TWebIQHidDeviceController.SetDeviceChangeEvent(const Notifier: TNotifyEvent);
begin
...
function TWebIQHidDeviceController.DoEnumerate(HidDev: THidDevice; Idx: Integer): Boolean;
begin
  Result := False;
...
procedure TWebIQHidDeviceController.SetEnumerate(const Enumerator: THidEnumerateEvent);
begin
...
procedure TWebIQHidDeviceController.SetDevThreadSleepTime(const DevTime: Integer);
var
  I: Integer;
  Dev: THidDevice;
begin
...
procedure TWebIQHidDeviceController.SetDevData(const DataEvent: THidDataEvent);
var
  I: Integer;
  Dev: THidDevice;
begin
...
procedure TWebIQHidDeviceController.SetDevDataError(const DataErrorEvent: THidDataErrorEvent);
var
  I: Integer;
  Dev: THidDevice;
begin
...
procedure TWebIQHidDeviceController.SetDevUnplug(const Unplugger: THidUnplugEvent);
var
  I: Integer;
  Dev: THidDevice;
begin
...
function TWebIQHidDeviceController.Enumerate: Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to FList.Count - 1 do
    if THidDevice(FList[I]).IsPluggedIn then
    begin
      Inc(Result);
...
// internal worker function to check out a THidDevice

function TWebIQHidDeviceController.CheckThisOut(var HidDev: THidDevice; Idx: Integer; Check: Boolean): Boolean;
begin
  Result := Check and not THidDevice(FList.Items[Idx]).IsCheckedOut;
  if Result then
  begin
    HidDev := FList[Idx];
...
function TWebIQHidDeviceController.CheckOutByProductName(var HidDev: THidDevice;
...
begin
  ...
  if ProductName <> '' then
    for I := 0 to FList.Count - 1 do
    begin
      Result := CheckThisOut(HidDev, I, ProductName = THidDevice(FList[I]).ProductName);
      if Result then
        Break;
    end;
end;
...
function TWebIQHidDeviceController.CheckOutByVendorName(var HidDev: THidDevice;
...
begin
  ...
  if VendorName <> '' then
    for I := 0 to FList.Count - 1 do
    begin
      Result := CheckThisOut(HidDev, I, VendorName = THidDevice(FList[I]).VendorName);
      if Result then
        Break;
    end;
end;
...
function TWebIQHidDeviceController.CheckOutByCallback(var HidDev: THidDevice;
  Check: THidCheckCallback): Boolean;
var
  I: Integer;
  Dev: THidDevice;
begin
...
function TWebIQHidDeviceController.CheckOutByClass(var HidDev: THidDevice;
...
begin
  ...
  if ClassName <> '' then
    for I := 0 to FList.Count - 1 do
    begin
      Result := CheckThisOut(HidDev, I, ClassName = THidDevice(FList[I]).PnPInfo.ClassDescr);
      if Result then
        Break;
    end;
end;
...
function TWebIQHidDeviceController.CheckOutByID(var HidDev: THidDevice;
...
begin
  ...
  for I := 0 to FList.Count - 1 do
  begin
    Result := CheckThisOut(HidDev, I, (Vid = THidDevice(FList[I]).Attributes.VendorID) and
      ((Pid = THidDevice(FList[I]).Attributes.ProductID) or (Pid = -1)));
    if Result then
      Break;
  end;
end;
...
function TWebIQHidDeviceController.CheckOutByIndex(var HidDev: THidDevice;
...
function TWebIQHidDeviceController.CheckOut(var HidDev: THidDevice): Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TWebIQHidDeviceController.CheckIn(var HidDev: THidDevice);
begin
  if HidDev <> nil then
  begin
    HidDev.StopThread;
...
function TWebIQHidDeviceController.CountByClass(const ClassName: string): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to FList.Count - 1 do
    if THidDevice(FList[I]).IsPluggedIn and
      (ClassName = THidDevice(FList[I]).PnPInfo.ClassDescr) then
      Inc(Result);
end;

function TWebIQHidDeviceController.CountByID(const Vid, Pid: Integer): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to FList.Count - 1 do
    if THidDevice(FList[I]).IsPluggedIn and
      (Vid = THidDevice(FList[I]).Attributes.VendorID) and
      ((Pid = THidDevice(FList[I]).Attributes.ProductID) or (Pid = -1)) then
      Inc(Result);
end;

function TWebIQHidDeviceController.CountByProductName(const ProductName: WideString): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to FList.Count - 1 do
    if THidDevice(FList[I]).IsPluggedIn and
      (ProductName = THidDevice(FList[I]).ProductName) then
      Inc(Result);
end;

function TWebIQHidDeviceController.CountByVendorName(const VendorName: WideString): Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to FList.Count - 1 do
    if THidDevice(FList[I]).IsPluggedIn and
      (VendorName = THidDevice(FList[I]).VendorName) then
      Inc(Result);
end;

function TWebIQHidDeviceController.CountByCallback(Check: THidCheckCallback): Integer;
var
  I: Integer;
  Dev: THidDevice;
begin
