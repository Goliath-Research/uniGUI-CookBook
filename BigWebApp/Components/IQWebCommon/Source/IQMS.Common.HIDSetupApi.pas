...
begin
  ...
  if SetupApiLoadCount > 1 then
    Exit;
  Result := IQMS.Common.ModuleLoader.LoadModule(SetupApiLib, SetupApiModuleName);
  if Result then
  begin
    {$IFDEF WINXP_UP}
...
begin
  ...
  if SetupApiLoadCount > 0 then
    Exit;
  IQMS.Common.ModuleLoader.UnloadModule(SetupApiLib);
  {$IFDEF WINXP_UP}
