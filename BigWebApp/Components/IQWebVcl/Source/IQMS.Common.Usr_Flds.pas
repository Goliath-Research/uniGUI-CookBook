...
//  IQXPMan,
  ...
  uniStatusBar,
  uniGUITypes, UniDBDateTimePicker,UniDateTimePicker,
  uniGUIAbstractClasses,
  uniGUIForm, UniMemo, UniDbMemo,
  uniGUIApplication,
  uniDBNavigator,
  uniLabel,uniComboBox,
  uniPageControl;
...
type
  ...
  TFrmUsr_Flds = class(TUniForm)
    DataSource1: TDataSource;
    pnlCarrier: TUniPanel;
    Panel2: TUniPanel;
    _sbtnStructure: TUniSpeedButton;
  ...
  private
    ...
    function Create_Label(const ATop: Integer; const AField: TField; ALabelsPanel: TUniPanel ): TUniLabel;
    function Create_Label_DBEdit_Pair(const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel ): TComponent;
    function Create_Label_DropDown_Pair(const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel; AStyle: TComboBoxStyle): TComponent;
    function Create_Label_wwDBDateTimePicker_Pair(const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel): TComponent;
    function Create_Label_CheckBox_Pair(const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel): TComponent;
    procedure AssignCaption;
    procedure CreateComponents( AUD_Groupbox_ID: Real; ALabelsPanel, AControlsPanel: TUniPanel );
    ...
    procedure AssignControlsToList( var AList: TList; AParent: TUniControl );
    ...
    function Create_Label_wwComboDlg(const ATop: Integer; AField: TField;
      ALabelsPanel, AControlsPanel: TUniPanel): TComponent;
    ...
    function GetScrollBoxOn(ATabsheet: TUniTabSheet): TUniScrollBox;
    function GetControlsPanelOn( AParent: TUniControl): TUniPanel;
  ...
  public
    { Public declarations }
    constructor Create(const ATable_Name, ASource: string;
  ...
  public
    constructor Create(AOwner: TComponent; AParent: TUniControl;
      const ATable_Name, ASource: string; AParent_ID: Real;
      AHelpContext: THelpContext = 0; ASR: TIQWebSecurityRegister = nil;
...
procedure Do_UD_Fields(const ATable_Name: string; AParent_ID: Real; AHelpContext: THelpContext = 0; ASource_Attribute: string = '' );
procedure AssignUD_MenuCaption(AMenuItem: TUniMenuItem; const ATable_Name: string; const ASource: string = ''; const ASource_Attribute: string = '' );
procedure AssignUD_SpeedButtonHint( ASpeedButton: TUniSpeedButton; const ATable_Name: string; const ASource_Attribute: string = '');
...
procedure AddUD_FieldsToPkList( AUD_Tables_ID: Real; APk: TIQWebHPick );
procedure CreateUD_PkListTFields( AUD_Tables_ID: Real; Apk: TIQWebHPick );
...
uses
  ...
  uniGroupBox,
  uniMultiItem,
  uniDBComboBox;  // for TwwDBComboBox.ButtonStyle
...
begin
  ...
  with TFrmUsr_Flds.Create( ATable_Name, ATable_Name {table_name a source for compatability}, AParent_ID, ASource_Attribute ) do
  try
...
procedure AssignUD_MenuCaption(AMenuItem: TUniMenuItem; const ATable_Name, ASource, ASource_Attribute: string);
var
  S: string;
begin
...
procedure AssignUD_SpeedButtonHint( ASpeedButton: TUniSpeedButton; const ATable_Name, ASource_Attribute: string);
var
  S: string;
begin
...
constructor TFrmUsr_Flds.Create( const ATable_Name,
...
//  if not (Owner is TIQWebUDEmbeddedForm) then  {embedded form calls the rebuild after pnlCarrier parent is changed}
...
begin
  ...
  for I:= ComponentCount - 1 downto 0 do
    if (Components[I].Tag < 0) and (Components[I] is TUniDBEdit) then with Components[I] as TUniDBEdit do
...
begin
  if SR = nil then
     SR:= TIQWebSecurityRegister.Create(self);
...
//      dbxUD_View.SQLConnection := IQMS.Common.DBDM.SQLConnection;
...
var
  ...
  ALastRowNum: Real;
  AScrollBox : TUniScrollBox;
  ATabsheetScrollBox : TUniScrollBox;

  ARowPanel: TUniPanel;
  AGroupBox: TUniGroupBox;

  ALabelsPanel  : TUniPanel;
  AControlsPanel: TUniPanel;

  {----------------------------------------------------------------------------}
  procedure _CreateRowPanel( var ARowPanel: TUniPanel; ARowNum: Real );
  begin
    // if this is new but not the first row panel - add a splitter aligned to top
    if Assigned(ARowPanel) then
       with TUniSplitter.Create(self) do
       begin
    ...
    // new row panel
    ARowPanel:= TUniPanel.Create(self);
    with ARowPanel do
    begin
      Parent := ATabsheetScrollBox;
  ...
  {----------------------------------------------------------------------------}
  procedure _CreateGroupBox( var AGroupBox: TUniGroupBox; var ALabelsPanel, AControlsPanel: TUniPanel; AUD_GroupBox_ID: Real; AName, ACaption: string );
  begin
    ...
    AGroupBox:= TUniGroupBox.Create( self );
    with AGroupBox do
    begin
      Parent := ARowPanel;
      Align  := alLeft;
    ...
    AScrollBox:= TUniScrollBox.Create( self );
    with AScrollBox do
    begin
      Parent    := AGroupBox;
      Align     := alClient;
    ...
    ALabelsPanel:= TUniPanel.Create(self);
    ...
    AControlsPanel:= TUniPanel.Create(self);
...
{------------------------------------------------------------------------------}
begin
  ATabsheet:= TUniTabSheet.Create( pgctrlMain );
  with ATabsheet do
  begin
  ...
  ATabsheetScrollBox:= TUniScrollBox.Create( self );
...
procedure TFrmUsr_Flds.CreateComponents( AUD_Groupbox_ID: Real; ALabelsPanel, AControlsPanel: TUniPanel );
var
  I         : Integer;
  ATop      : Integer;
...
begin
  ...
  IQMS.Common.Controls.AdjustRightAlignedControlsToParent( AControlsPanel );

  if AControlsPanel.Parent is TUniScrollBox then
     TUniScrollBox(AControlsPanel.Parent).VertScrollBar.Range:= ATop;
end;
...
begin
  ...
  for I:= ComponentCount - 1 downto 0 do
    if (Components[I].Tag < 0) and not ((Components[I] is TUniPanel) or (Components[I] is TUniGroupBox)) then   {all added on fly components like labels and dbedits have tag = -1}
    begin
      {remove from SR}
...
procedure TFrmUsr_Flds.AssignControlsToList( var AList: TList; AParent: TUniControl );
var
  I: Integer;
begin
  for I:= 0 to AParent.ControlCount - 1 do
  begin
    if (AParent.Controls[ I ] is TUniPanel) or (AParent.Controls[ I ] is TUniGroupBox) or (AParent.Controls[ I ] is TUniScrollBox) then
       AssignControlsToList( AList, AParent.Controls[ I ] as TUniControl );  // recursion
  ...
  try
    ...
    begin
      ...
      for J:= 0 to AControlsList.Count - 1 do
        if (TObject(AControlsList[ J ]) is TUniGroupBox) or (TObject(AControlsList[ J ]) is TUniPanel) then
...
function TFrmUsr_Flds.GetScrollBoxOn( ATabsheet: TUniTabSheet ): TUniScrollBox;
var
  I: Integer;
begin
  Result:= nil;
  for I:= 0 to ATabsheet.ControlCount - 1 do
    if (ATabsheet.Controls[ I ] is TUniScrollBox) and (CompareText(Copy(TUniScrollBox(ATabsheet.Controls[ I ]).Name, 1, 19 ), 'tabsheet_scrollbox_') = 0 ) then
    begin
      Result:= TUniScrollBox(ATabsheet.Controls[ I ]);
      BREAK;
    end;
end;

function TFrmUsr_Flds.GetControlsPanelOn( AParent: TUniControl): TUniPanel;
var
  I: Integer;
begin
  Result:= nil;
  for I:= 0 to AParent.ControlCount - 1 do
    if (AParent.Controls[ I ] is TUniScrollBox) then
       begin
         Result:= GetControlsPanelOn( TUniControl(AParent.Controls[ I ]));  // recursion
         BREAK;
       end
    else if (AParent.Controls[ I ] is TUniPanel) and (CompareText(Copy(TUniPanel(AParent.Controls[ I ]).Name, 1, 13 ), 'pnl_controls_') = 0 ) then
      begin
        Result:= TUniPanel(AParent.Controls[ I ]);
        BREAK;
      end;
end;
...
var
  ...
  ATotalWidth : Integer;
  AScrollBox: TUniScrollBox;
  AZeroWidthList: TList<TUniPanel>;
  APanel: TUniPanel;
  ARowPanel: TUniPanel;
  AStyle: NativeInt;

  // ---------------------------------------------------------------------------
  function _GetTotalGroupBoxesWidthOn( ARowPanel: TUniPanel ): Integer;
  var
    I: Integer;
    AControlsPanel: TUniPanel;
    AGroupBox: TUniGroupBox;
  begin
    ...
    for I:= 0 to ARowPanel.ControlCount -1 do
      if ARowPanel.Controls[ i ] is TUniGroupBox then
         begin
           AGroupBox:= TUniGroupBox(ARowPanel.Controls[ i ]);
      ...
      else if ARowPanel.Controls[ i ] is TUniSplitter then with TUniSplitter(ARowPanel.Controls[ i ]) do
         begin
  ...
  // ---------------------------------------------------------------------------
  function _GetMaxGroupboxesHeight( ARowPanel: TUniPanel ): Integer;
  var
    I,J: Integer;
    AControlsPanel: TUniPanel;
    AGroupBox: TUniGroupBox;
...
begin
  ...
  AControlsList:= TList.Create;
  AZeroWidthList:= TList<TUniPanel>.Create;
  try
    for I:= 0 to pgctrlMain.PageCount - 1 do
    begin
      ATabsheet:= pgctrlMain.Pages[ I ];
      ...
      begin
        ...
        if (TObject(AControlsList[ J ]) is TUniPanel) and (CompareText( Copy( TUniPanel(AControlsList[ J ]).Name, 1, 8), 'pnl_row_') = 0) then
           begin
             ARowPanel:= TUniPanel( AControlsList[ J ]);
        ...
        else if (TObject(AControlsList[ J ]) is TUniSplitter) then with TUniSplitter( AControlsList[ J ]) do
           begin
             if Align = alTop then
...
function TFrmUsr_Flds.Create_Label(const ATop: Integer; const AField: TField; ALabelsPanel: TUniPanel ): TUniLabel;
begin
  // Initialization
  Result := NIL;
  // Create the label
...
function TFrmUsr_Flds.Create_Label_DBEdit_Pair(const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel ): TComponent;
var
   ALabel: TUniLabel;
begin
...
function TFrmUsr_Flds.Create_Label_wwComboDlg( const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel ): TComponent;
var
   ALabel: TUniLabel;
begin
  ALabel := Create_Label(ATop, AField, ALabelsPanel);
...
begin
  ...
  if (Sender is TUniDBComboBox) then with TUniDBComboBox(Sender) do
    begin
...
begin
  ...
  if not Assigned(AParent.Component) or not (AParent.Component is TUniDBComboBox) then
     EXIT;
  ...
  Result:= SelectValueFmtAsFloat('select id from ud_drop_down_text where ud_cols_id = %f and text = ''%s''',
                     [ AThis.UD_Cols_Parent_ID,                                // parent ud_cols_id
                       FixStr( TUniDBComboBox( AParent.Component ).Text )]);   // parent text value
end;
...
procedure TFrmUsr_Flds.LoadDropDownValues( Sender: TObject; AUD_Col_ID: Real );
  ...
  begin
    ...
    AList:= TUniDBComboBox( Sender).Items;
    AList.Clear;
    with TFDQuery.Create(NIL) do
    try
      ConnectionName := 'IQFD';
  ...
  begin
   ...
   TUniDBComboBox( Sender).OnDropDown:= ComboBoxDropDownInvokePicklist;
//   TUniDBComboBox( Sender).ButtonStyle:= cbsEllipsis;
  end;
...
var
  ...
  begin
    if (AComponent is TUniDBComboBox) then with TUniDBComboBox(AComponent) do
    begin
      if Assigned(DataSource) and Assigned(DataSource.DataSet) and (DataField <> '') and (DataSource.DataSet.State in [dsEdit, dsInsert]) then
         try
           ...
           TUniDBComboBox(AComponent).Text:= '';
         finally
...
begin
  ...
  try
    try
      ...
      begin
        IQMS.Common.ErrorDialog.ShowIQErrorMessage( IQMS.Common.ResStrings.cTXT0000482, E.Message, ''); // 'Unable to reset dependent user defined drop-down boxes';
        EXIT;
      end;
    end;
...
function TFrmUsr_Flds.Create_Label_DropDown_Pair(const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel; AStyle: TComboBoxStyle): TComponent;
var
   ALabel: TUniLabel;
...
begin
  ...
  Result:= TUniDBComboBox.Create(self);
  with TUniDBComboBox(Result) do
  begin
    Name  := Format('_ud_component_%s', [AField.FieldName]);
    Parent:= AControlsPanel;
    Left  := 0;
...
//  ALabel.FocusControl := TUniDBComboBox(Result);
  IQMS.Common.Controls.CenterLabel(TUniDBComboBox(Result), ALabel);
...
function TFrmUsr_Flds.Create_Label_wwDBDateTimePicker_Pair(const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel ): TComponent;
var
   ALabel: TUniLabel;
begin
  ALabel := Create_Label(ATop, AField, ALabelsPanel);
...
function TFrmUsr_Flds.Create_Label_CheckBox_Pair( const ATop: Integer; AField: TField; ALabelsPanel, AControlsPanel: TUniPanel ): TComponent;
var
  ALabel: TUniLabel;
begin
  ALabel := Create_Label(ATop, AField, ALabelsPanel);
  ...
  Result:= TUniDBCheckBox.Create(self);
  with TUniDBCheckBox(Result) do
  begin
    Name          := Format('_ud_component_%s', [AField.FieldName]);
    Parent        := AControlsPanel;
    Left          := 0;
...
//  ALabel.FocusControl:= TUniDBCheckBox(Result);
  IQMS.Common.Controls.IQAlignControls( [TUniDBCheckBox(Result), ALabel], IQMS.Common.Controls.acVerticalCenters);
end;
...
begin
  ...
  if (Sender is TUniDBEdit) then with (Sender as TUniDBEdit).Field do
     begin
       DataSet.Edit;
       Clear;
     end
  else if Sender is TUniDBDateTimePicker then with TUniDBDateTimePicker(Sender), DataSource do
     begin
       DataSet.Edit;
...
procedure TFrmUsr_Flds.FormControlsReadOnly(const AReadOnly: Boolean);
  ...
  begin
    for i := 0 to AWinControl.ControlCount - 1 do
     begin
       if (AWinControl.Controls[i] is TUniLabel) then
         TUniLabel(AWinControl.Controls[i]).Enabled := not AReadOnly
       else if (AWinControl.Controls[i] is TComponent) then
         IQMS.Common.Controls.IQReadOnlyControl(AWinControl.Controls[i], AReadOnly);
...
begin
  if (Source > '') and (Source_Attribute > '') then
    Result:= Format('%s_%s', [ Source, IQMS.Common.StringUtils.FixComponentName( Source_Attribute )])
  else if Source > '' then
    Result := Source
  else if Table_Name > '' then
    Result := Table_Name
    ...
    Result := '';
{
  if Table_Name = '' then
     ...
     Result:= Format('%s_%s', [ Table_Name, IQMS.Common.StringUtils.FixComponentName( Source_Attribute )]);}
end;
...
var
  ...
  ADataSet: TDataset;
begin
  if not (Sender is TUniDbEdit) then
     EXIT;

  with TUniDbEdit(Sender) do
  begin
    ADataSet:= DataSource.DataSet;
    ...
    if ReadOnly then
       DoViewMemoStr(  S )
    else if IQMS.Common.EditMemoStr.DoEditMemoStr(S, 250 ) then
       begin
         ADataSet.Edit;
...
begin
  ...
  begin
    ...
    if not ReadOnly then
       IQMS.Common.Controls.CheckClearWWDBComboDlg( Sender, Key, ADataSet.FieldByName( DataField ));
  end;
end;
...
constructor TFrmUsr_Flds_BuiltIn.Create(AOwner: TComponent;
  AParent: TUniControl; const ATable_Name, ASource: string; AParent_ID: Real;
  AHelpContext: THelpContext = 0; ASR: TIQWebSecurityRegister = nil; ASource_Attribute: string = '');
begin
  SR:= ASR;
  inherited Create( ATable_Name, ASource, AParent_ID, ASource_Attribute);
  HelpContext:= AHelpContext;
...
//  ForceRefreshSecurityInspector; // refreshes the SR items
  ...
  if Owner is TIQWebUDEmbeddedForm then
     PostMessage(TIQWebUDEmbeddedForm(Owner).Handle, iq_Notify, 0, 0);
end;
...
begin
  ...
  if Owner is TUniForm then
     PostMessage(TUniForm(Owner).Handle, iq_Notify, 0, 0);
