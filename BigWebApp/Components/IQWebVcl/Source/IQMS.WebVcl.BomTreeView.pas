...
uses
  ...
  Vcl.DBCtrls, Vcl.Printers, FireDAC.Comp.Client, FireDAC.Comp.DataSet,
...
//    CS       : TRTLCriticalSection;
    ...
    procedure CheckSubNodesLoaded( Node : TTreeNode );
    function LoadRootNode( AParent: TTreeNode; AID, AQty: Real ):TTreeNode; virtual; abstract;
    procedure LoadSubNodes( AParent:TTreeNode; AArinvtID, AStandardID : Real ); virtual; abstract;
...
//     procedure Expand(Node: TTreeNode); override;
  ...
  private
    ...
    function LoadRootNode( AParent: TTreeNode; AID, AQty: Real ):TTreeNode; override;
    procedure LoadSubNodes( AParent:TTreeNode; AArinvtID, AStandardID : Real ); override;
    procedure CheckAppendAsBlend( AParent:TTreeNode; AArinvtID, AStandardID : Real );
    procedure CheckAppendMainPL( AParent:TTreeNode; AArinvtID, AStandardID : Real );
    procedure CheckAppendAsOpMat( AParent:TTreeNode; AArinvtID, AStandardID : Real );
    procedure AppendNodesFrom( AQuery:TFDQuery; AParent:TTreeNode; AArinvtID, AStandardID : Real );
...
begin
  ...
  Qty           := AQty;
  UOM_Factor    := IfThen(AUOM_Factor = 0, 1, AUOM_Factor);
end;

{ TIQWebBomTreeViewBase }
...
//  try
//    FIs_Metric_Sys:= IsMetricSys;  {IQUom.pas}
//  except
...
//procedure TIQWebBomTreeViewBase.Change(Node: TTreeNode);
//begin
//  try
//    EnterCriticalSection( CS );
//    if Assigned( Node.Data ) then
...
//procedure TIQWebBomTreeViewBase.Expand(Node: TTreeNode);
//begin
//  try
//    EnterCriticalSection( CS );
//    if Assigned( Node.Data ) then
...
procedure TIQWebBomTreeViewBase.CheckSubNodesLoaded( Node : TTreeNode );
begin
  if not( csDesigning in ComponentState ) then
...
function TIQWebBomTreeView.LoadRootNode( AParent: TTreeNode; AID, AQty: Real ):TTreeNode;
//var
//  AStandardID: Real;
//  AItemNo    : string;
begin
...
//
//    Result:= Items.AddChildObject( AParent, AItemNo, TIQWebTreeNodeCargo.Create( AID, AStandardID, fIIf( AQty = 0, 1, AQty )));
//  end
//  else
    Result:= NIL;
end;

procedure TIQWebBomTreeView.LoadSubNodes( AParent:TTreeNode; AArinvtID, AStandardID : Real );
begin
//  if not( csDesigning in ComponentState ) then
//  try
...
procedure TIQWebBomTreeView.CheckAppendAsOpMat( AParent:TTreeNode; AArinvtID, AStandardID : Real );
//var
//  AQuery: TFDQuery;
begin
...
procedure TIQWebBomTreeView.CheckAppendAsBlend( AParent:TTreeNode; AArinvtID, AStandardID : Real );
//var
//  AQuery: TFDQuery;
begin
//  AQuery:= TFDQuery.Create(nil);
...
procedure TIQWebBomTreeView.CheckAppendMainPL( AParent:TTreeNode; AArinvtID, AStandardID: Real );
//var
//  AQuery          : TFDQuery;
...
procedure TIQWebBomTreeView.AppendNodesFrom( AQuery:TFDQuery; AParent:TTreeNode; AArinvtID, AStandardID : Real );
//var
//  Node         : TTreeNode;
//  S            : string;
//  APtsPer      : Real;
...
//                   AQty:= IQRoundEx( TIQWebTreeNodeCargo( AParent.Data ).Qty * TIQWebTreeNodeCargo( AParent.Data ).UOM_Factor * 1 / FieldByName('ptsper').asFloat, 6)  {IQNumber.pas}
//                else
//                   AQty:= 0;
//              end
//           else
...
//                // AQty:= SelectValueFmtAsFloat('select UOM.To_Native( %f, ''%s'') from dual', [ TIQWebTreeNodeCargo( AParent.Data ).Qty, SelectValueByID('uom', 'standard', AStandardID) ]);
...
//         ANonCommitted:= inv_misc.GetCalculateNonComitted( FieldByName('arinvt_id').asFloat );
...
//       AUOM_Factor:= 1;
//
//    Node:= Items.AddChildObject( AParent,
//                                 S,
//                                 TIQWebTreeNodeCargo.Create( FieldByName('arinvt_id').asFloat,
...
begin
  ...
  try
    ...
    PrintDefaultReportEx( self, 'BOM_TREE.RPT', SelectionList );  {in IqPrint.pas}
  finally
    SelectionList.Free;
  end;
end;
...
//
//  procedure _AddLine(ANode: TTreeNode; AFont: TFont);
//  var
//    sl: TStringList;
//    ADivisor: Real;
...
//    try
//      sl.Text := iqstring.BlockWrapText(
//        ANode.Text,
//        ANodeLevel,
//        c_max_line_width
//        );
