unit IQMS.Common.ProdDateShift;
...
//    end;
  ...
  // as usual.
  if (AProdShift = 0) or (AProdDate = MinValue) then
  begin
...
//    end
  ...
  Result := (AProdShift > 0) and (AProdDate > MinValue);
end;
...
procedure IQAssertProdDateShiftFound(const AProdShift: Integer;
  const AProdDate: TDateTime);
begin
  if (AProdShift <= 0) and (AProdDate <= MinValue) then
    // IQMS.Common.ResourceStrings.cTXT0000448 =
