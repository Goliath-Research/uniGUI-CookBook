...
type
  ...
  private
    ...
    procedure SetIQWebAbout(aIQWebAbout:TIQWebAbout);
  public
    property IQWebAbout : TIQWebAbout read FIQWebAbout write SetIQWebAbout;
    { Public declarations }
  end;

procedure DoIQAboutBox(aIQWebAbout:TIQWebAbout);
...
uses
  IQMS.WebVcl.ABoutAlter,
  IQMS.Common.TimeDiffDlg,
  IQMS.Common.DataLib,
...
begin
  ...
  IQAboutBox.IQWebAbout := aIQWebAbout;
  IQAboutBox.ShowModal;
end;

procedure TIQAboutBox.SetIQWebAbout(aIQWebAbout:TIQWebAbout);
begin
    FIQWebAbout := aIQWebAbout;
...
//   lblDotNetVersion.Caption := DotNetVersionName[iqsysinfo.DotNetVersion];

  if (SecurityManager <> nil) then
    lblUserName.Caption:= SecurityManager.UserName
  else
    lblUserName.Caption:= cTXT0000051; // 'N/A'
...
begin
  with TIQWebAbout( Owner ) do
    ...
    if IQConfirmYN( cTXT0000054 ) then
  ...
  if (SecurityManager <> nil) then
    begin
       ...
       try
         ...
         ABackEnd:= cTXT0000051; // 'N/A'
       end;
    end
  else
    begin
      ALicensed:= cTXT0000055; // 'N/A (Security Manager is Unavailable)'
      ABackEnd := cTXT0000055; // 'N/A (Security Manager is Unavailable)'
    end;
  {'IQ Replication Manager Status:'#13+
   'Licensed: %s'#13 +
   'Active (Del Log Enabled): %s'}
  IQInformation(Format( cTXT0000056, [ ALicensed, ABackEnd ]));
end;
...
begin
  ...
  TTimeDiff.Execute( Format(cTXT0000057, [ ATimeDiff * 24 ]), ATimeDiff );
end;
...
begin
  ...
  try
    IQWebAbout.GetSupportEmailBodyText(ABodyText);
