...
type
  TIQDesignerSelectForm = class(TUniForm)
...
{ TIQDesignerSelectForm }

procedure TIQDesignerSelectForm.SetCategory(const Value: Integer);
begin
  FCategory := Value;
end;

procedure TIQDesignerSelectForm.SetData(const Value: Variant);
begin
  FData := Value;
end;

procedure TIQDesignerSelectForm.SetItemIndex(const Value: Integer);
begin
  FItemIndex := Value;
end;

procedure TIQDesignerSelectForm.SetResult(const Value: string);
begin
  FResult := Value;
end;

procedure TIQDesignerSelectForm.SetResultClass(const Value: TPersistentClass);
begin
  FResultClass := Value;
end;

procedure TIQDesignerSelectForm.ShowList(const ACaption: string;
...
procedure TIQDesignerSelectForm.ShowList(const ACaption: string; Callback: TUniDialogCallBackProc);
begin
  ShowList(ACaption,
    procedure(Sender: TComponent; AResult:Integer)
    begin
...
procedure TIQDesignerSelectForm.ShowList(const ACaption: string; const AList: TStrings; Callback: TUniDialogCallBackProc);
var
  I, Index : Integer;
  Sl : TUniControlClass;
begin
...
procedure TIQDesignerSelectForm.UniFormBeforeShow(Sender: TObject);
begin
  if UniListBox.Items.Count > 0 then
    UniListBox.ItemIndex := 0;
end;

procedure TIQDesignerSelectForm.UniListBoxDblClick(Sender: TObject);
begin
  ModalResult := mrOK;
end;
