...
type
  ...
  TIQDataServicesFireDAC_Oracle = class(TInterfacedObject, IIQDataServices)
  protected
...
{ TIQDataServicesFireDAC_Oracle }

procedure TIQDataServicesFireDAC_Oracle.SortDataSet
(
  const aDataSet          : TDataSet;
  const aFieldName        : string;
  const aAscending        : boolean;
...
function TIQDataServicesFireDAC_Oracle.GetIQMSConnection: TCustomConnection;
begin
  Result := ServiceLocator.GetService<TIQMS_Params>.IQMSConnection;
end;

function  TIQDataServicesFireDAC_Oracle.CloneConnection(const aConnection   : TCustomConnection) : TCustomConnection;
begin
...
function TIQDataServicesFireDAC_Oracle.GetNextSeqVal
(
  const aConnection   : TCustomConnection;
  const aSequenceName : string
) : Variant;
...
function TIQDataServicesFireDAC_Oracle.GetQuery(const aSQL : string) : TDataSet;
var
  Qry : TFDQuery;
begin
...
function TIQDataServicesFireDAC_Oracle.CloneDataSet
(
  const aDataSet      : TDataSet;
        aReset        : boolean = false;
        aKeepSettings : boolean = false
...
function TIQDataServicesFireDAC_Oracle.GetSortingCriteria(
...
procedure TIQDataServicesFireDAC_Oracle.SetParamValue(const aDataSet: TDataSet;
...
procedure TIQDataServicesFireDAC_Oracle.SetSortingCriteria(
...
function TIQDataServicesFireDAC_Oracle.SortingCriteriaToString(
...
function TIQDataServicesFireDAC_Oracle.StringToSortingCriteria(
  const aIndexFieldNames: string): IDataSetSortingCriteria;
begin

end;

function TIQDataServicesFireDAC_Oracle.LoadBlob
(
  const aConnection : TCustomConnection;
  const aTable,
        aKeyField,
...
procedure TIQDataServicesFireDAC_Oracle.NotifyOnAfterScroll(const aDataSet: TDataSet; DoOnAfterScroll: TDataSetNotifyEvent);
begin
  Guard.CheckInheritsFrom(aDataSet, TFDDataSet, 'ADataSet');
...
function TIQDataServicesFireDAC_Oracle.RefreshRecord(const aDataSet: TDataSet): boolean;
begin
  Guard.CheckInheritsFrom(aDataSet, TFDDataSet, 'ADataSet');
...
procedure TIQDataServicesFireDAC_Oracle.SaveBlob
(
  const aConnection : TCustomConnection;
  const aTable,
        aKeyField,
