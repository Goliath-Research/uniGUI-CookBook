...
procedure ShowIQShipManDebug(AError, AXMLRequest, AXMLResponse: String);
var
  FrmShipManDebug : TFrmShipManDebug;
begin
  FrmShipManDebug := TFrmShipManDebug.Create(uniGUIApplication.UniApplication);
  with FrmShipManDebug do
  begin
     memError.Text    := AError;
     memRequest.Text  := StrTran(AXMLRequest, #10, #13#10);
     memResponse.Text := StrTran(AXMLResponse, #10, #13#10);
...
S : TStringStream;
begin
  S := TStringStream.Create(memRequest.Lines.Text);
    try
      UniSession.SendStream(S, 'XML Request.txt');
    finally
      S.Free;
    end;
end;
...
S : TStringStream;
begin
  S := TStringStream.Create(memResponse.Lines.Text);
    try
      UniSession.SendStream(S, 'XML Response.txt');
    finally
      S.Free;
    end;
end;
