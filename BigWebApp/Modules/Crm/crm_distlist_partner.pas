...
type
  TFrmCRMDistListPartner = class(TFrmCRMDistList)
    QryMasterID: TBCDField;
    QryMasterPARTNERNO: TStringField;
    QryMasterCOMPANY: TStringField;
    QryMasterDISTLIST: TStringField;
    QryDistListID: TBCDField;
    QryDistListPARTNER_ID: TBCDField;
...
{ TFrmCRMDistListPartner }

procedure TFrmCRMDistListPartner.InitValues;
begin
  MasterSource := 'PARTNER';
  TableName := 'PARTNER_DIST';
  ParentFieldName := 'PARTNER_ID';

  if not QryMaster.Active then
    ReOpen(QryMaster);
  SourceDisplay := Format('%s - %s',
    [Trim(QryMaster.FieldByName('PARTNERNO').AsString),
