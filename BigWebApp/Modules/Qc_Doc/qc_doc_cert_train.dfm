object FrmQcDocCertTraining: TFrmQcDocCertTraining
  Left = 0
  Top = 0
  HelpContext = 20096
  ClientHeight = 518
  ClientWidth = 1207
  Caption = 'Certification Training'
  OnShow = UniFormShow
  OldCreateOrder = False
  OnClose = FormClose
  OnActivate = FormActivate
  Menu = MainMenu1
  MonitoredKeys.Keys = <>
  PixelsPerInch = 96
  TextHeight = 13
  object pnlCarrier: TUniPanel
    Left = 0
    Top = 0
    Width = 1207
    Height = 518
    Hint = ''
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Align = alClient
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 0
    Caption = ''
    object Splitter1: TUniSplitter
      Left = 305
      Top = 0
      Width = 5
      Height = 518
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Align = alLeft
      ParentColor = False
      Color = clBtnFace
    end
    object Panel3: TUniPanel
      Left = 310
      Top = 1
      Width = 897
      Height = 516
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Align = alClient
      Anchors = [akLeft, akTop, akRight, akBottom]
      TabOrder = 0
      Caption = ''
      object Splitter2: TUniSplitter
        Left = 0
        Top = 284
        Width = 897
        Height = 5
        Cursor = crVSplit
        Hint = ''
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        Align = alTop
        ParentColor = False
        Color = clBtnFace
      end
      object pnlTop01: TUniPanel
        Left = 1
        Top = 1
        Width = 895
        Height = 284
        Hint = ''
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        Align = alTop
        Anchors = [akLeft, akTop, akRight]
        TabOrder = 0
        Caption = ''
        object Panel6: TUniPanel
          Left = 1
          Top = 1
          Width = 893
          Height = 27
          Hint = ''
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          Align = alTop
          Anchors = [akLeft, akTop, akRight]
          TabOrder = 0
          Caption = ''
          object Panel4: TUniPanel
            Left = 858
            Top = 1
            Width = 35
            Height = 25
            Hint = ''
            Margins.Left = 4
            Margins.Top = 4
            Margins.Right = 4
            Margins.Bottom = 4
            ShowHint = True
            ParentShowHint = False
            Align = alRight
            Anchors = [akTop, akRight, akBottom]
            TabOrder = 0
            Caption = ''
            ExplicitLeft = 633
            object Shape10: TUniPanel
              Left = 4
              Top = 4
              Width = 17
              Height = 17
              Hint = 'Missing correct answer'
              Margins.Left = 4
              Margins.Top = 4
              Margins.Right = 4
              Margins.Bottom = 4
              ShowHint = True
              TabOrder = 1
              Caption = ''
            end
          end
        end
        inline wwDBGridQuestions: TIQUniGridControl
          Left = 1
          Top = 27
          Width = 893
          Height = 257
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          Layout = 'border'
          ParentAlignmentControl = False
          AlignmentControl = uniAlignmentClient
          Align = alClient
          Anchors = [akLeft, akTop, akRight, akBottom]
          TabOrder = 1
          ExplicitLeft = 1
          ExplicitTop = 27
          ExplicitWidth = 893
          ExplicitHeight = 257
          inherited PageControl: TUniPageControl
            Width = 893
            Height = 228
            ExplicitWidth = 893
            ExplicitHeight = 228
            inherited GridTab: TUniTabSheet
              ExplicitLeft = 4
              ExplicitTop = 24
              ExplicitWidth = 885
              ExplicitHeight = 200
              inherited DBGrid: TUniDBGrid
                Width = 676
                Height = 171
                Columns = <>
              end
              inherited GridEditorsPanel: TUniHiddenPanel
                Left = 676
                Height = 171
                ExplicitLeft = 676
                ExplicitHeight = 171
              end
              inherited GridToolBar: TUniToolBar
                Width = 885
                ExplicitWidth = 885
              end
            end
            inherited FormTab: TUniTabSheet
              ExplicitLeft = 4
              ExplicitTop = 24
              ExplicitWidth = 885
              ExplicitHeight = 200
            end
          end
          inherited TopToolBar: TUniToolBar
            Width = 893
            ExplicitWidth = 893
          end
          inherited ImgList: TUniNativeImageList
            Images = {
              
              426082}
          end
        end
        object wwDBComboDlgQuestion: TUniEdit
          Left = 126
          Top = 89
          Width = 189
          Height = 21
          Hint = ''
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          Text = ''
          TabOrder = 2
        end
        object wwDBComboDlgCorrectAnswer: TUniEdit
          Left = 131
          Top = 129
          Width = 184
          Height = 21
          Hint = ''
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          Text = ''
          TabOrder = 3
          ReadOnly = True
        end
      end
      object Panel5: TUniPanel
        Left = 1
        Top = 289
        Width = 895
        Height = 227
        Hint = ''
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        Align = alClient
        Anchors = [akLeft, akTop, akRight, akBottom]
        TabOrder = 1
        Caption = ''
        object Panel7: TUniPanel
          Left = 1
          Top = 1
          Width = 893
          Height = 27
          Hint = ''
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          Align = alTop
          Anchors = [akLeft, akTop, akRight]
          TabOrder = 0
          Caption = ''
        end
        inline wwDBGridAnswers: TIQUniGridControl
          Left = 1
          Top = 27
          Width = 893
          Height = 200
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          Layout = 'border'
          ParentAlignmentControl = False
          AlignmentControl = uniAlignmentClient
          Align = alClient
          Anchors = [akLeft, akTop, akRight, akBottom]
          TabOrder = 1
          ExplicitLeft = 1
          ExplicitTop = 27
          ExplicitWidth = 893
          ExplicitHeight = 200
          inherited PageControl: TUniPageControl
            Width = 893
            Height = 171
            ExplicitWidth = 893
            ExplicitHeight = 171
            inherited GridTab: TUniTabSheet
              ExplicitLeft = 4
              ExplicitTop = 24
              ExplicitWidth = 885
              ExplicitHeight = 143
              inherited DBGrid: TUniDBGrid
                Width = 676
                Height = 114
                Columns = <>
              end
              inherited GridEditorsPanel: TUniHiddenPanel
                Left = 676
                Height = 114
                ExplicitLeft = 676
                ExplicitHeight = 114
              end
              inherited GridToolBar: TUniToolBar
                Width = 885
                ExplicitWidth = 885
              end
            end
            inherited FormTab: TUniTabSheet
              ExplicitLeft = 4
              ExplicitTop = 24
              ExplicitWidth = 885
              ExplicitHeight = 143
            end
          end
          inherited TopToolBar: TUniToolBar
            Width = 893
            ExplicitLeft = 1
            ExplicitTop = -6
            ExplicitWidth = 893
          end
          inherited ImgList: TUniNativeImageList
            Images = {
              
              426082}
          end
        end
        object wwDBComboDlgAnswer: TUniEdit
          Left = 131
          Top = 115
          Width = 158
          Height = 21
          Hint = ''
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          Text = ''
          TabOrder = 2
        end
      end
    end
    object pnlLeft01: TUniPanel
      Left = 1
      Top = 1
      Width = 305
      Height = 516
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Align = alLeft
      Anchors = [akLeft, akTop, akBottom]
      TabOrder = 1
      Caption = ''
      object Panel2: TUniPanel
        Left = 1
        Top = 1
        Width = 303
        Height = 27
        Hint = ''
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        Align = alTop
        Anchors = [akLeft, akTop, akRight]
        TabOrder = 0
        Caption = ''
        object navMain: TUniDBNavigator
          Left = 203
          Top = 1
          Width = 100
          Height = 25
          Hint = ''
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          DataSource = SrcCertTraining
          VisibleButtons = [nbDelete, nbPost, nbCancel, nbRefresh]
          ConfirmDelete = False
          Align = alRight
          Anchors = [akTop, akRight, akBottom]
          TabOrder = 0
        end
      end
      object wwDataInspector1: TwwDataInspector
        Left = 0
        Top = 27
        Width = 305
        Height = 489
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        DisableThemes = False
        Align = alClient
        TabOrder = 2
        DataSource = SrcCertTraining
        Items = <
          item
            DataSource = SrcCertTraining
            DataField = 'ID'
            Caption = 'ID'
            ReadOnly = True
            WordWrap = False
          end
          item
            DataSource = SrcCertTraining
            DataField = 'DESCRIP'
            Caption = 'Description'
            PickList.ButtonStyle = cbsEllipsis
            WordWrap = False
          end
          item
            DataSource = SrcCertTraining
            DataField = 'PASS_PERCENT'
            Caption = 'Pass Percent'
            WordWrap = False
          end>
        DefaultRowHeight = 20
        CaptionWidth = 101
        Options = [ovColumnResize, ovRowResize, ovEnterToTab, ovHighlightActiveRow, ovCenterCaptionVert]
        CaptionFont.Charset = DEFAULT_CHARSET
        CaptionFont.Color = clWindowText
        CaptionFont.Height = -11
        CaptionFont.Name = 'Tahoma'
        CaptionFont.Style = []
        LineStyleCaption = ovDottedLine
        LineStyleData = ovDottedLine
      end
    end
  end
  object QryCertTraining: TFDQuery
    BeforeOpen = QryCertTrainingBeforeOpen
    AfterPost = ApplyUpdates
    AfterDelete = ApplyUpdates
    CachedUpdates = True
    Connection = UniMainModule.FDConnection1
    UpdateObject = UpdateSQLCertTraining
    SQL.Strings = (
      'select id,'
      '       descrip,'
      '       pass_percent'
      '  from cert_training'
      ' where id = :cert_training_id')
    Left = 20
    Top = 136
    ParamData = <
      item
        Name = 'cert_training_id'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object QryCertTrainingID: TBCDField
      FieldName = 'ID'
      Origin = 'IQ.CERT_TRAINING.ID'
      Size = 0
    end
    object QryCertTrainingDESCRIP: TStringField
      FieldName = 'DESCRIP'
      Origin = 'IQ.CERT_TRAINING.DESCRIP'
      Size = 50
    end
    object QryCertTrainingPASS_PERCENT: TBCDField
      FieldName = 'PASS_PERCENT'
      Origin = 'IQ.CERT_TRAINING.PASS_PERCENT'
      Size = 0
    end
  end
  object SrcCertTraining: TDataSource
    DataSet = QryCertTraining
    Left = 20
    Top = 108
  end
  object MainMenu1: TUniMainMenu
    Left = 20
    Top = 212
    object File1: TUniMenuItem
      Caption = 'File'
      object Exit1: TUniMenuItem
        Caption = 'Exit'
        OnClick = Exit1Click
      end
    end
    object Reports1: TUniMenuItem
      Caption = '&Reports'
      object Print1: TUniMenuItem
        Caption = '&Print'
        OnClick = Print1Click
      end
      object PrinterSetup1: TUniMenuItem
        Caption = 'Printer &Setup'
        OnClick = PrinterSetup1Click
      end
    end
    object Help: TUniMenuItem
      Caption = 'Help'
      object Contents1: TUniMenuItem
        Caption = 'Contents'
        OnClick = Contents1Click
      end
      object About1: TUniMenuItem
        Caption = 'About'
        OnClick = About1Click
      end
    end
  end
  object IQAbout1: TIQWebAbout
    ModuleName = 'FrmQcDocCertTraining'
    Version = 'DEV.176554'
    ChangeListNumber = '$Change: 192757 $'
    BuildVersion = '176554'
    Left = 56
    Top = 212
  end
  object QryCertTrainQuestions: TFDQuery
    BeforeInsert = PostParentBeforeEdit
    BeforeEdit = PostParentBeforeEdit
    BeforePost = QryCertTrainQuestionsBeforePost
    AfterPost = ApplyUpdates
    AfterDelete = ApplyUpdates
    OnCalcFields = QryCertTrainQuestionsCalcFields
    OnNewRecord = QryCertTrainQuestionsNewRecord
    CachedUpdates = True
    MasterSource = SrcCertTraining
    MasterFields = 'ID'
    DetailFields = 'ID'
    Connection = UniMainModule.FDConnection1
    FetchOptions.AssignedValues = [evCache]
    FetchOptions.Cache = [fiBlobs, fiMeta]
    UpdateObject = UpdateSQLQuestion
    SQL.Strings = (
      'select id,'
      '       cert_training_id,'
      '       seq,'
      '       text,'
      '       cert_training_answer_id'
      '  from cert_training_question       '
      ' where cert_training_id = :id'
      ' order by seq')
    Left = 288
    Top = 96
    ParamData = <
      item
        Name = 'ID'
        DataType = ftBCD
        ParamType = ptInput
        Size = 34
        Value = Null
      end>
    object QryCertTrainQuestionsSEQ: TBCDField
      DisplayLabel = '#'
      DisplayWidth = 6
      FieldName = 'SEQ'
      Origin = 'IQ.CERT_TRAINING_QUESTION.SEQ'
      Size = 0
    end
    object QryCertTrainQuestionsTEXT: TStringField
      DisplayLabel = 'Question'
      DisplayWidth = 50
      FieldName = 'TEXT'
      Origin = 'IQ.CERT_TRAINING_QUESTION.TEXT'
      Size = 250
    end
    object QryCertTrainQuestionsCorrectAnswerText: TStringField
      DisplayLabel = 'Correct Answer'
      DisplayWidth = 50
      FieldKind = fkCalculated
      FieldName = 'CorrectAnswerText'
      Size = 250
      Calculated = True
    end
    object QryCertTrainQuestionsID: TBCDField
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'IQ.CERT_TRAINING_QUESTION.ID'
      Visible = False
      Size = 0
    end
    object QryCertTrainQuestionsCERT_TRAINING_ID: TBCDField
      DisplayWidth = 10
      FieldName = 'CERT_TRAINING_ID'
      Origin = 'IQ.CERT_TRAINING_QUESTION.CERT_TRAINING_ID'
      Visible = False
      Size = 0
    end
    object QryCertTrainQuestionsCERT_TRAINING_ANSWER_ID: TBCDField
      DisplayWidth = 10
      FieldName = 'CERT_TRAINING_ANSWER_ID'
      Origin = 'IQ.CERT_TRAINING_QUESTION.CERT_TRAINING_ANSWER_ID'
      Visible = False
      Size = 0
    end
  end
  object SrcCertTrainQuestions: TDataSource
    DataSet = QryCertTrainQuestions
    Left = 288
    Top = 68
  end
  object QryCertTrainAnswers: TFDQuery
    BeforeInsert = PostParentBeforeEdit
    BeforeEdit = PostParentBeforeEdit
    BeforePost = QryCertTrainAnswersBeforePost
    AfterPost = ApplyUpdates
    AfterDelete = ApplyUpdates
    OnNewRecord = QryCertTrainAnswersNewRecord
    CachedUpdates = True
    MasterSource = SrcCertTrainQuestions
    MasterFields = 'ID'
    DetailFields = 'ID'
    Connection = UniMainModule.FDConnection1
    FetchOptions.AssignedValues = [evCache]
    FetchOptions.Cache = [fiBlobs, fiMeta]
    UpdateObject = UpdateSQLAnswer
    SQL.Strings = (
      'select id,'
      '       cert_training_question_id,'
      '       seq,'
      '       text'
      '  from cert_training_answer      '
      'where cert_training_question_id = :id'
      ' order by seq')
    Left = 288
    Top = 316
    ParamData = <
      item
        Name = 'ID'
        DataType = ftBCD
        ParamType = ptInput
        Size = 8
      end>
    object QryCertTrainAnswersSEQ: TBCDField
      DisplayLabel = '#'
      DisplayWidth = 6
      FieldName = 'SEQ'
      Origin = 'IQ.CERT_TRAINING_ANSWER.SEQ'
      Size = 0
    end
    object QryCertTrainAnswersTEXT: TStringField
      DisplayLabel = 'Answer'
      DisplayWidth = 75
      FieldName = 'TEXT'
      Origin = 'IQ.CERT_TRAINING_ANSWER.TEXT'
      Size = 250
    end
    object QryCertTrainAnswersID: TBCDField
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'IQ.CERT_TRAINING_ANSWER.ID'
      Visible = False
      Size = 0
    end
    object QryCertTrainAnswersCERT_TRAINING_QUESTION_ID: TBCDField
      DisplayWidth = 10
      FieldName = 'CERT_TRAINING_QUESTION_ID'
      Origin = 'IQ.CERT_TRAINING_ANSWER.CERT_TRAINING_QUESTION_ID'
      Visible = False
      Size = 0
    end
  end
  object SrcCertTrainAnswers: TDataSource
    DataSet = QryCertTrainAnswers
    Left = 288
    Top = 288
  end
  object UpdateSQLQuestion: TFDUpdateSQL
    Connection = UniMainModule.FDConnection1
    InsertSQL.Strings = (
      'insert into cert_training_question'
      '  (ID, CERT_TRAINING_ID, SEQ, TEXT, CERT_TRAINING_ANSWER_ID)'
      'values'
      
        '  (:ID, :CERT_TRAINING_ID, :SEQ, :TEXT, :CERT_TRAINING_ANSWER_ID' +
        ')')
    ModifySQL.Strings = (
      'update cert_training_question'
      'set'
      '  ID = :ID,'
      '  CERT_TRAINING_ID = :CERT_TRAINING_ID,'
      '  SEQ = :SEQ,'
      '  TEXT = :TEXT,'
      '  CERT_TRAINING_ANSWER_ID = :CERT_TRAINING_ANSWER_ID'
      'where'
      '  ID = :OLD_ID')
    DeleteSQL.Strings = (
      'delete from cert_training_question'
      'where'
      '  ID = :OLD_ID')
    Left = 288
    Top = 124
  end
  object UpdateSQLAnswer: TFDUpdateSQL
    Connection = UniMainModule.FDConnection1
    InsertSQL.Strings = (
      'insert into cert_training_answer'
      '  (ID, CERT_TRAINING_QUESTION_ID, SEQ, TEXT)'
      'values'
      '  (:ID, :CERT_TRAINING_QUESTION_ID, :SEQ, :TEXT)')
    ModifySQL.Strings = (
      'update cert_training_answer'
      'set'
      '  ID = :ID,'
      '  CERT_TRAINING_QUESTION_ID = :CERT_TRAINING_QUESTION_ID,'
      '  SEQ = :SEQ,'
      '  TEXT = :TEXT'
      'where'
      '  ID = :OLD_ID')
    DeleteSQL.Strings = (
      'delete from cert_training_answer'
      'where'
      '  ID = :OLD_ID')
    Left = 288
    Top = 344
  end
  object PkAnswer: TIQWebHpick
    BeforeOpen = PkAnswerBeforeOpen
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select id,'
      '       seq,'
      '       text'
      '  from cert_training_answer      '
      ' where cert_training_question_id = :id'
      ' order by seq')
    Title = 'Select from Pick List'
    Touchscreen = False
    ForceShowWaitForPrompt = False
    Left = 448
    Top = 124
    ParamData = <
      item
        Name = 'id'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object PkAnswerID: TBCDField
      FieldName = 'ID'
      Origin = 'IQ.CERT_TRAINING_ANSWER.ID'
      Visible = False
      Size = 0
    end
    object PkAnswerSEQ: TBCDField
      DisplayLabel = '#'
      DisplayWidth = 6
      FieldName = 'SEQ'
      Origin = 'IQ.CERT_TRAINING_ANSWER.SEQ'
      Size = 0
    end
    object PkAnswerTEXT: TStringField
      DisplayLabel = 'Answer'
      DisplayWidth = 50
      FieldName = 'TEXT'
      Origin = 'IQ.CERT_TRAINING_ANSWER.TEXT'
      Size = 250
    end
  end
  object UpdateSQLCertTraining: TFDUpdateSQL
    Connection = UniMainModule.FDConnection1
    InsertSQL.Strings = (
      'insert into cert_training'
      '  (ID, DESCRIP, PASS_PERCENT)'
      'values'
      '  (:ID, :DESCRIP, :PASS_PERCENT)')
    ModifySQL.Strings = (
      'update cert_training'
      'set'
      '  ID = :ID,'
      '  DESCRIP = :DESCRIP,'
      '  PASS_PERCENT = :PASS_PERCENT'
      'where'
      '  ID = :OLD_ID')
    DeleteSQL.Strings = (
      'delete from cert_training'
      'where'
      '  ID = :OLD_ID')
    Left = 20
    Top = 164
  end
  object IQRepDef1: TIQWebRepDef
    App_ID = 'FrmQcDocCertTraining'
    WebUse = False
    TouchScreen = False
    Left = 88
    Top = 212
  end
  object PrinterSetupDialog1: TPrinterSetupDialog
    Left = 119
    Top = 212
  end
  object SR: TIQWebSecurityRegister
    ForceAccessItems.Strings = (
      'FRMQCDOCCERTTRAINING')
    SecurityCode = 'FRMQCDOCCERTTRAINING'
    Left = 155
    Top = 212
  end
end
