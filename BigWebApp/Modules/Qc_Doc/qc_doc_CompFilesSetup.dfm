object FrmFileCmprSetup: TFrmFileCmprSetup
  Left = 335
  Top = 208
  ClientHeight = 453
  ClientWidth = 626
  Caption = 'Setup File Comparison Utilities'
  Position = poOwnerFormCenter
  OldCreateOrder = False
  BorderIcons = [biSystemMenu]
  MonitoredKeys.Keys = <>
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter1: TUniSplitter
    Left = 0
    Top = 171
    Width = 626
    Height = 3
    Cursor = crVSplit
    Hint = ''
    Align = alBottom
    ParentColor = False
    Color = clBtnFace
  end
  object Panel1: TUniPanel
    Left = 0
    Top = 0
    Width = 626
    Height = 171
    Hint = ''
    Align = alClient
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 0
    Caption = 'Panel1'
    object Panel2: TUniPanel
      Left = 1
      Top = 1
      Width = 624
      Height = 27
      Hint = ''
      ParentShowHint = False
      Align = alTop
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 0
      Caption = ''
    end
    inline wwDBGrid1: TIQUniGridControl
      Left = 1
      Top = 27
      Width = 624
      Height = 144
      Layout = 'border'
      ParentAlignmentControl = False
      AlignmentControl = uniAlignmentClient
      Align = alClient
      Anchors = [akLeft, akTop, akRight, akBottom]
      TabOrder = 1
      ExplicitLeft = 1
      ExplicitTop = 27
      ExplicitWidth = 624
      ExplicitHeight = 144
      inherited PageControl: TUniPageControl
        Width = 624
        Height = 115
        ExplicitWidth = 624
        ExplicitHeight = 115
        inherited GridTab: TUniTabSheet
          ExplicitLeft = 4
          ExplicitTop = 24
          ExplicitWidth = 616
          ExplicitHeight = 87
          inherited DBGrid: TUniDBGrid
            Width = 407
            Height = 58
            Columns = <>
          end
          inherited GridEditorsPanel: TUniHiddenPanel
            Left = 407
            Height = 58
            ExplicitLeft = 407
            ExplicitHeight = 58
          end
          inherited GridToolBar: TUniToolBar
            Width = 616
            ExplicitWidth = 616
          end
        end
        inherited FormTab: TUniTabSheet
          ExplicitLeft = 4
          ExplicitTop = 24
          ExplicitWidth = 616
          ExplicitHeight = 87
        end
      end
      inherited TopToolBar: TUniToolBar
        Width = 624
        ExplicitWidth = 624
      end
      inherited ImgList: TUniNativeImageList
        Images = {
          
          426082}
      end
    end
    object wwDBComboBox1: TUniDBComboBox
      Left = 196
      Top = 122
      Width = 121
      Height = 21
      Hint = ''
      Items.Strings = (
        'Use Default Comparison')
      TabOrder = 2
      OnDropDown = wwDBComboBox1DropDown
    end
  end
  object Panel3: TUniPanel
    Left = 0
    Top = 174
    Width = 626
    Height = 129
    Hint = ''
    Align = alBottom
    Anchors = [akLeft, akRight, akBottom]
    TabOrder = 1
    Caption = 'Panel3'
    object GroupBox2: TUniGroupBox
      Left = 1
      Top = 1
      Width = 624
      Height = 127
      Hint = ''
      Caption = 'Test Selected Command Line Exe Syntax'
      Align = alClient
      Anchors = [akLeft, akTop, akRight, akBottom]
      TabOrder = 0
      DesignSize = (
        624
        127)
      object Label2: TUniLabel
        Left = 8
        Top = 19
        Width = 71
        Height = 13
        Hint = ''
        Caption = '%FileName1%'
        TabOrder = 3
      end
      object Label3: TUniLabel
        Left = 8
        Top = 51
        Width = 71
        Height = 13
        Hint = ''
        Caption = '%FileName2%'
        TabOrder = 4
      end
      object sbtnSelectFileName1: TUniSpeedButton
        Left = 585
        Top = 16
        Width = 20
        Height = 20
        Hint = ''
        Glyph.Data = {
          
          0888888888888888888888888888888888888888888888888888}
        Caption = ''
        Anchors = [akTop, akRight]
        ParentColor = False
        Color = clWindow
        OnClick = btnSelectFileName1Click
      end
      object sbtnSelectFileName2: TUniSpeedButton
        Left = 585
        Top = 47
        Width = 20
        Height = 20
        Hint = ''
        Glyph.Data = {
          
          0888888888888888888888888888888888888888888888888888}
        Caption = ''
        Anchors = [akTop, akRight]
        ParentColor = False
        Color = clWindow
        OnClick = btnSelectFileName2Click
      end
      object edFile1: TUniEdit
        Left = 80
        Top = 16
        Width = 497
        Height = 21
        Hint = ''
        Text = ''
        Anchors = [akLeft, akTop, akRight]
        TabOrder = 0
      end
      object edFile2: TUniEdit
        Left = 80
        Top = 47
        Width = 497
        Height = 21
        Hint = ''
        Text = ''
        Anchors = [akLeft, akTop, akRight]
        TabOrder = 1
      end
      object btnCompare: TUniButton
        Left = 494
        Top = 78
        Width = 79
        Height = 24
        Hint = ''
        Caption = 'Test'
        Anchors = [akTop, akRight]
        TabOrder = 7
        OnClick = btnTestCompareClick
      end
    end
  end
  object CaptionPanel1: TIQWebCaptionPanel
    Left = 0
    Top = 303
    Width = 626
    Height = 150
    Hint = 
      'For more information regarding the Command Line Execution Syntax' +
      ', please see the help files.'
    Align = alBottom
    Anchors = [akLeft, akRight, akBottom]
    TabOrder = 3
    Caption = 'Example'
    ParentColor = True
    Text = 
      'For files with an extension of .TXT, enter TXT for the File Ext.' +
      '  For example, if the application you use to compare text files ' +
      'is ExamDiff, then the '#8216'Cmd Line Exe Syntax'#8217' might look like: '#8220'C:' +
      '\Program Files\ExamDiff\ExamDiff.exe" "%FileName1%" "%FileName2%' +
      '" where %FileName1% and %FileName2% are variables that Enterpris' +
      'eIQ will replace with the file names that are being compared.  T' +
      'he variables should not be changed.  Optional command line flags' +
      ' are supported and can be placed before, after or in between the' +
      ' variables.  Please refer to the help files of your file compari' +
      'son application for the correct syntax.  To default all file ext' +
      'ensions to a particular application, enter a single asterisk for' +
      ' the File Ext.  To launch Microsoft Word'#39's comparison utility, s' +
      'elect '#8216'Use Internal Comparison'#8217' from the '#8216'Command Line Execution' +
      ' Syntax'#8217' drop down box.  You can use the form above to test your' +
      ' setup.'
    PictureLayout = plImageRight
    LayoutStyle = lsWindowsXP
    DesignSize = (
      626
      150)
  end
  object OpenDialog1: TOpenDialog
    Left = 365
    Top = 310
  end
  object srcDoc_Compare: TDataSource
    DataSet = tblDoc_Compare
    Left = 88
    Top = 80
  end
  object tblDoc_Compare: TFDTable
    BeforePost = tblDoc_CompareBeforePost
    Connection = UniMainModule.FDConnection1
    UpdateOptions.AssignedValues = [uvUpdateMode]
    UpdateOptions.UpdateTableName = 'DOC_COMPARE'
    TableName = 'DOC_COMPARE'
    Left = 96
    Top = 89
    object tblDoc_CompareFILE_EXT: TStringField
      DisplayLabel = 'File Ext.'
      DisplayWidth = 8
      FieldName = 'FILE_EXT'
      Required = True
      EditMask = '>C!LLL;0; '
      Size = 5
    end
    object tblDoc_CompareCOMMAND: TStringField
      DisplayLabel = 'Command Line Execution Syntax'
      DisplayWidth = 88
      FieldName = 'COMMAND'
      Size = 200
    end
    object tblDoc_CompareID: TBCDField
      DisplayWidth = 10
      FieldName = 'ID'
      Visible = False
      Size = 0
    end
  end
  object OpenDialog2: TOpenDialog
    DefaultExt = 'EXE'
    Filter = '*.exe|*.exe|*.*|*.*'
    Options = [ofHideReadOnly, ofFileMustExist, ofEnableSizing]
    Left = 137
    Top = 89
  end
  object popmGrid: TUniPopupMenu
    Left = 169
    Top = 89
    object FindExecutable1: TUniMenuItem
      Caption = 'Find Executable ...'
      OnClick = FindExecutable1Click
    end
  end
end
