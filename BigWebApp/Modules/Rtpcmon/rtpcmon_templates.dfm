object FrmDeviceTemplate: TFrmDeviceTemplate
  Left = 424
  Top = 131
  ClientHeight = 570
  ClientWidth = 798
  Caption = 'Device Templates'
  OldCreateOrder = False
  OnClose = FormClose
  Menu = MainMenu1
  MonitoredKeys.Keys = <>
  OnSubmit = UniFormSubmit
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter1: TUniSplitter
    Left = 0
    Top = 258
    Width = 798
    Height = 3
    Cursor = crVSplit
    Hint = ''
    Align = alTop
    ParentColor = False
    Color = clBtnFace
  end
  object pnlDeviceToolbar: TUniPanel
    Tag = 1999
    Left = 0
    Top = 0
    Width = 798
    Height = 27
    Hint = ''
    Align = alTop
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 0
    Caption = ''
    object sbtnSearchDeviceTemplates: TUniSpeedButton
      Left = 1
      Top = 1
      Width = 25
      Height = 25
      Hint = 'Search for Template'
      ShowHint = True
      ParentShowHint = False
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Caption = ''
      ParentColor = False
      Color = clWindow
      OnClick = sbtnSearchDeviceTemplatesClick
    end
    object Bevel1: TUniPanel
      Left = 29
      Top = 3
      Width = 2
      Height = 21
      Hint = ''
      TabOrder = 3
      Caption = ''
    end
    object DBNavigator1: TUniDBNavigator
      Left = 573
      Top = 1
      Width = 225
      Height = 25
      Hint = ''
      ShowHint = True
      ParentShowHint = False
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbInsert, nbDelete, nbPost, nbCancel, nbRefresh]
      Align = alRight
      Anchors = [akTop, akRight, akBottom]
      TabOrder = 0
    end
  end
  object pnlDeviceCapabilities: TUniPanel
    Left = 0
    Top = 261
    Width = 798
    Height = 309
    Hint = ''
    Align = alClient
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 1
    Caption = ''
    object gbxCapabilities: TUniGroupBox
      Left = 1
      Top = 1
      Width = 796
      Height = 307
      Hint = ''
      Caption = '  Device Capabilities  '
      Align = alClient
      Anchors = [akLeft, akTop, akRight, akBottom]
      TabOrder = 0
      object wwDBComboDlgCapabType: TUniEdit
        Left = 16
        Top = 72
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 0
      end
      object wwDBComboDlgResultPrefix: TUniEdit
        Left = 16
        Top = 120
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 1
      end
      object wwDBComboDlgCommand: TUniEdit
        Left = 16
        Top = 96
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 2
      end
      object wwDBComboDlgResultPostfix: TUniEdit
        Left = 16
        Top = 144
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 3
      end
      object wwDBComboDlgPacketHeader: TUniEdit
        Left = 16
        Top = 168
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 4
      end
      object wwDBComboDlgResponseAck: TUniEdit
        Left = 16
        Top = 192
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 5
      end
      object Panel2: TUniPanel
        Left = 2
        Top = 15
        Width = 792
        Height = 290
        Hint = ''
        Align = alClient
        Anchors = [akLeft, akTop, akRight, akBottom]
        TabOrder = 6
        Caption = ''
        ExplicitTop = 27
        ExplicitHeight = 280
        object pctrlCapab: TUniPageControl
          Left = 1
          Top = 1
          Width = 790
          Height = 288
          Hint = ''
          ActivePage = tabCapabProtocol
          TabBarVisible = True
          Align = alClient
          Anchors = [akLeft, akTop, akRight, akBottom]
          TabOrder = 0
          ExplicitHeight = 278
          object tabCapabProtocol: TUniTabSheet
            Hint = ''
            Caption = 'tabCapabProtocol'
            ExplicitHeight = 250
            inline wwDBGrid1: TIQUniGridControl
              Left = 0
              Top = 0
              Width = 782
              Height = 260
              Layout = 'border'
              ParentAlignmentControl = False
              AlignmentControl = uniAlignmentClient
              Align = alClient
              Anchors = [akLeft, akTop, akRight, akBottom]
              TabOrder = 0
              ExplicitWidth = 782
              ExplicitHeight = 250
              inherited PageControl: TUniPageControl
                Width = 782
                Height = 231
                ExplicitWidth = 782
                ExplicitHeight = 221
                inherited GridTab: TUniTabSheet
                  inherited DBGrid: TUniDBGrid
                    Width = 565
                    Height = 174
                    Columns = <>
                  end
                  inherited GridEditorsPanel: TUniHiddenPanel
                    Left = 565
                    Height = 174
                  end
                  inherited GridToolBar: TUniToolBar
                    Width = 774
                  end
                end
              end
              inherited TopToolBar: TUniToolBar
                Width = 782
                inherited DBNavigator: TUniDBNavigator
                  Top = -2
                  ExplicitTop = -2
                end
              end
              inherited ImgList: TUniNativeImageList
                Images = {
                  
                  426082}
              end
            end
            object cmbCommandName: TUniDBComboBox
              Left = 120
              Top = 104
              Width = 121
              Height = 21
              Hint = ''
              Style = csDropDownList
              TabOrder = 1
              OnCloseUp = cmbCommandNameCloseUp
              OnEnter = cmbCommandNameEnter
            end
          end
          object tabCapabCustom: TUniTabSheet
            Hint = ''
            ImageIndex = 1
            Caption = 'tabCapabCustom'
            ExplicitHeight = 250
            inline grdDeviceCapabilites: TIQUniGridControl
              Left = 0
              Top = 0
              Width = 782
              Height = 260
              Layout = 'border'
              ParentAlignmentControl = False
              AlignmentControl = uniAlignmentClient
              Align = alClient
              Anchors = [akLeft, akTop, akRight, akBottom]
              TabOrder = 0
              ExplicitWidth = 782
              ExplicitHeight = 250
              inherited PageControl: TUniPageControl
                Width = 782
                Height = 231
                ExplicitWidth = 782
                ExplicitHeight = 221
                inherited GridTab: TUniTabSheet
                  inherited DBGrid: TUniDBGrid
                    Width = 565
                    Height = 174
                    Columns = <>
                  end
                  inherited GridEditorsPanel: TUniHiddenPanel
                    Left = 565
                    Height = 174
                  end
                  inherited GridToolBar: TUniToolBar
                    Width = 774
                  end
                end
              end
              inherited TopToolBar: TUniToolBar
                Width = 782
              end
              inherited ImgList: TUniNativeImageList
                Images = {
                  
                  426082}
              end
            end
          end
        end
      end
    end
  end
  object pnlDeviceInfo: TUniPanel
    Left = 0
    Top = 27
    Width = 798
    Height = 231
    Hint = ''
    Align = alTop
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 2
    Caption = ''
    object lblDeviceName: TUniLabel
      Left = 8
      Top = 8
      Width = 59
      Height = 13
      Hint = ''
      Caption = 'Device Type'
      TabOrder = 3
    end
    object edDeviceName: TUniDBEdit
      Left = 80
      Top = 4
      Width = 161
      Height = 21
      Hint = ''
      DataField = 'NAME'
      TabOrder = 0
    end
    object pctrlDevice: TUniPageControl
      Left = 1
      Top = 32
      Width = 796
      Height = 199
      Hint = ''
      ActivePage = tabProtocol
      TabBarVisible = True
      Align = alBottom
      Anchors = [akLeft, akTop, akRight, akBottom]
      TabOrder = 1
      object tabProtocol: TUniTabSheet
        Hint = ''
        ImageIndex = 2
        Caption = 'Protocol'
        object DBCheckBox3: TUniDBCheckBox
          Left = 8
          Top = 3
          Width = 158
          Height = 17
          Hint = 'The byte order of the data section of a packet is little endian.'
          ShowHint = True
          ParentShowHint = False
          DataField = 'LITTLE_ENDIAN'
          ValueChecked = 'Y'
          ValueUnchecked = 'N'
          Caption = 'Packet Data is Little Endian '
          TabOrder = 0
        end
        object cmbProtocol: TUniDBComboBox
          Left = 8
          Top = 32
          Width = 210
          Height = 21
          Hint = ''
          DataField = 'PROTOCOL'
          DataSource = DM_RTPCMon.srcPCMON_DEVICE_TEMPLATE
          Style = csDropDownList
          Sorted = True
          TabOrder = 1
        end
      end
      object TabSheet2: TUniTabSheet
        Hint = ''
        ImageIndex = 2
        Caption = 'Connection'
        object DBCheckBox2: TUniDBCheckBox
          Left = 8
          Top = 19
          Width = 158
          Height = 17
          Hint = 'The device sends data at it'#39's interval to the PC.'
          ShowHint = True
          ParentShowHint = False
          DataField = 'ALWAYS_CONNECTED'
          ValueChecked = 'Y'
          ValueUnchecked = 'N'
          Caption = 'Always Connected'
          TabOrder = 0
        end
        object DBCheckBox1: TUniDBCheckBox
          Left = 8
          Top = 3
          Width = 158
          Height = 17
          Hint = 
            'The device requests the connection to the PC.  Otherwise the PC ' +
            'will request connection to the device.'
          ShowHint = True
          ParentShowHint = False
          DataField = 'INITS_CONNECT'
          ValueChecked = 'Y'
          ValueUnchecked = 'N'
          Caption = 'Device Initiates Connection'
          TabOrder = 1
        end
      end
      object tabOfflineCommand: TUniTabSheet
        Hint = ''
        ImageIndex = 2
        Caption = 'Offline Command'
        object Label2: TUniLabel
          Left = 8
          Top = 12
          Width = 47
          Height = 13
          Hint = ''
          Caption = 'Command'
          TabOrder = 2
        end
        object Label1: TUniLabel
          Left = 8
          Top = 36
          Width = 47
          Height = 13
          Hint = ''
          Caption = 'Response'
          TabOrder = 3
        end
        object cmbOfflineCommand: TUniEdit
          Left = 88
          Top = 8
          Width = 201
          Height = 21
          Hint = ''
          Text = ''
          TabOrder = 0
        end
        object cmbOfflineResponse: TUniEdit
          Left = 88
          Top = 32
          Width = 201
          Height = 21
          Hint = ''
          Text = ''
          TabOrder = 1
        end
      end
    end
  end
  object MainMenu1: TUniMainMenu
    Left = 352
    object File1: TUniMenuItem
      Caption = '&File'
      object Export1: TUniMenuItem
        Caption = '&Export Templates'
        OnClick = Export1Click
      end
      object ImportTemplates1: TUniMenuItem
        Caption = 'Import Templates'
        OnClick = ImportTemplates1Click
      end
      object N1: TUniMenuItem
        Caption = '-'
      end
      object Close1: TUniMenuItem
        Caption = '&Close'
        OnClick = Close1Click
      end
    end
    object Settings1: TUniMenuItem
      Caption = '&Settings'
      object ServerDirectorySetup1: TUniMenuItem
        Caption = '&Server Directory Setup'
        OnClick = ServerDirectorySetup1Click
      end
    end
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = 'xml'
    Filter = 'XML File|*.xml'
    Options = [ofOverwritePrompt, ofHideReadOnly, ofEnableSizing]
    Left = 104
    Top = 8
  end
  object OpenDialog1: TOpenDialog
    DefaultExt = 'xml'
    Filter = 'XML File|*.xml'
    Left = 80
    Top = 8
  end
  object IQAbout1: TIQWebAbout
    ModuleName = 'FrmDeviceTemplate'
    Version = 'DEV.176554'
    ChangeListNumber = '$Change: 195786 $'
    BuildVersion = '176554'
    Left = 272
    Top = 51
  end
end
