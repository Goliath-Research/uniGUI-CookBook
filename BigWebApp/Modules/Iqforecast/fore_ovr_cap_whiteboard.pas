...
type
  ...
  TFrmForeOvrCapWhiteboard = class(TUniForm)
    Panel1: TUniPanel;
    ...
    Bevel1: TUniPanel;
    PkCenterType: TIQWebHPick;
    PkCenterTypeCNTR_TYPE: TStringField;
    PkCenterTypeMFGTYPE: TStringField;
    PkCenterTypeMFGCELL: TStringField;
    PkCenterTypeMFGCELL_ID: TBCDField;
...
procedure TFrmForeOvrCapWhiteboard.UniFormCreate(Sender: TObject);
begin
...
procedure TFrmForeOvrCapWhiteboard.FormClose(Sender: TObject;
...
procedure TFrmForeOvrCapWhiteboard.wwDBGridPlanDrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
begin
...
procedure TFrmForeOvrCapWhiteboard.DrawPeriodInGrid( AGrid: TIQUniGridControl; Rect: TRect; AReqHrs, AAvailHrs: Real );
var
  ARectLabelReq    : TRect;
...
procedure TFrmForeOvrCapWhiteboard.DrawCenterTypeDetails( AGrid: TIQUniGridControl; Rect: TRect; ACenterType, AMfgType, AMfgCell: string );
var
...
procedure TFrmForeOvrCapWhiteboard.QryMonthlyBeforeOpen(DataSet: TDataSet);
begin
...
//  end;
  ...
  AssignQueryParamValue(DataSet, 'fore_head_id', FFore_Head_ID);
end;

procedure TFrmForeOvrCapWhiteboard.wwDBGridPlanCalcCellColors(Sender: TObject;
...
procedure TFrmForeOvrCapWhiteboard.AbortOnInsert(DataSet: TDataSet);
begin
  ABORT;
end;

procedure TFrmForeOvrCapWhiteboard.QryMonthlyUpdateRecord(ASender: TDataSet; ARequest: TFDUpdateRequest; var AAction: TFDErrorAction; AOptions: TFDUpdateRowOptions);

begin
  AAction := eaApplied;
end;

procedure TFrmForeOvrCapWhiteboard.CheckRefresh(Sender: TObject;  Button: TNavigateBtn);
begin
...
procedure TFrmForeOvrCapWhiteboard.RefreshActiveDataSet; // obsolete
var
...
procedure TFrmForeOvrCapWhiteboard.Close1Click(Sender: TObject);
begin
  Close;
end;

procedure TFrmForeOvrCapWhiteboard.About1Click(Sender: TObject);
begin
  IQAbout1.Execute;
end;


procedure TFrmForeOvrCapWhiteboard.UpdateGridPeriodsTitles;
var
...
procedure TFrmForeOvrCapWhiteboard.FormShow(Sender: TObject);
begin
  // caption
  self.Caption:= Format('Forecast Whiteboard Capacity Plan: %s', [ SelectValueByID('name', 'fore_head', FFore_Head_ID)]);
  IQSetTablesActive( TRUE, self );
  UpdateGridPeriodsTitles;
end;

function TFrmForeOvrCapWhiteboard.ActiveDataSet: TFDQuery;
begin
  case GetViewType of
...
function TFrmForeOvrCapWhiteboard.GetStartDate: TDateTime;
var
  ADate: TDateTime;
begin
...
function TFrmForeOvrCapWhiteboard.GetViewType: TCapacityWhiteboardView;
begin
...
procedure TFrmForeOvrCapWhiteboard.sbtnApplyClick(Sender: TObject);
var
  hMesg: TPanelMesg;
begin
  // Validate
...
procedure TFrmForeOvrCapWhiteboard.ApplyCriteria;
var
  AIsOpen: Boolean;
  ACntrType, AMfgType: string;
  AMfgCell_ID: Real;
...
procedure TFrmForeOvrCapWhiteboard.sbtnBackClick(Sender: TObject);
begin
  MovePeriod( -1 {move 1 period back} );
end;

procedure TFrmForeOvrCapWhiteboard.sbtnForwardClick(Sender: TObject);
begin
  MovePeriod( +1 {move 1 period forward} );
end;

procedure TFrmForeOvrCapWhiteboard.MovePeriod( ADirection: Integer );
var
  ADelta: Real;
begin
  case GetViewType of
...
procedure TFrmForeOvrCapWhiteboard.bbtnSearchClick(Sender: TObject);
begin
...
procedure TFrmForeOvrCapWhiteboard.wwDBComboBoxViewTypeChange(Sender: TObject);
begin
  QryMonthly.Close;
  QryWeekly.Close;
  QryDaily.Close;
...
procedure TFrmForeOvrCapWhiteboard.FormActivate(Sender: TObject);
begin
  IQHelp.AssignHTMLFile( iqchmForecast{CHM}, iqchmForecast{HTM} );  {IQMS.Common.HelpHook.pas}
end;

procedure TFrmForeOvrCapWhiteboard.Contents1Click(Sender: TObject);
begin
