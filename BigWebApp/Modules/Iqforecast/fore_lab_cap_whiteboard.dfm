object FrmForeLabCapWhiteboard: TFrmForeLabCapWhiteboard
  Left = 96
  Top = 203
  HelpContext = 17901
  ClientHeight = 480
  ClientWidth = 1178
  Caption = 'Forecast Labor Whiteboard Capacity Plan'
  OnShow = FormShow
  OldCreateOrder = False
  OnClose = FormClose
  OnActivate = FormActivate
  Menu = MainMenu1
  MonitoredKeys.Keys = <>
  Font.Height = -14
  OnCreate = UniFormCreate
  PixelsPerInch = 96
  TextHeight = 17
  object Splitter1: TUniSplitter
    Left = 238
    Top = 0
    Width = 6
    Height = 480
    Hint = ''
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Align = alLeft
    ParentColor = False
    Color = clBtnFace
  end
  object Panel1: TUniPanel
    Left = 244
    Top = 0
    Width = 934
    Height = 480
    Hint = ''
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Align = alClient
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 0
    Caption = ''
    ExplicitWidth = 938
    object wwDBGridPlan: TIQUniGridControl
      Left = 1
      Top = 36
      Width = 932
      Height = 444
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Layout = 'border'
      ParentAlignmentControl = False
      AlignmentControl = uniAlignmentClient
      Align = alClient
      Anchors = [akLeft, akTop, akRight, akBottom]
      Font.Height = -14
      TabOrder = 0
      IQComponents.Grid.Left = 0
      IQComponents.Grid.Top = 29
      IQComponents.Grid.Width = 715
      IQComponents.Grid.Height = 358
      IQComponents.Grid.Hint = ''
      IQComponents.Grid.DataSource = SrcCapacityPlan
      IQComponents.Grid.LoadMask.Message = 'Loading data...'
      IQComponents.Grid.Align = alClient
      IQComponents.Grid.Anchors = [akLeft, akTop, akRight, akBottom]
      IQComponents.Grid.TabOrder = 0
      IQComponents.Navigator.Left = 410
      IQComponents.Navigator.Top = 0
      IQComponents.Navigator.Width = 241
      IQComponents.Navigator.Height = 25
      IQComponents.Navigator.Hint = ''
      IQComponents.Navigator.ShowHint = True
      IQComponents.Navigator.DataSource = SrcCapacityPlan
      IQComponents.Navigator.LayoutConfig.Region = 'east'
      IQComponents.Navigator.TabOrder = 3
      IQComponents.HiddenPanel.Left = 715
      IQComponents.HiddenPanel.Top = 29
      IQComponents.HiddenPanel.Width = 209
      IQComponents.HiddenPanel.Height = 358
      IQComponents.HiddenPanel.Hint = ''
      IQComponents.HiddenPanel.Visible = True
      IQComponents.HiddenPanel.Align = alRight
      IQComponents.HiddenPanel.Anchors = [akTop, akRight, akBottom]
      IQComponents.HiddenPanel.ExplicitLeft = 719
      IQComponents.FormTab.Hint = ''
      IQComponents.FormTab.ImageIndex = 1
      IQComponents.FormTab.Caption = 'Form'
      IQComponents.FormTab.Layout = 'border'
      IQComponents.FormTab.ExplicitLeft = 0
      IQComponents.FormTab.ExplicitTop = 0
      IQComponents.FormTab.ExplicitWidth = 928
      IQComponents.FormTab.ExplicitHeight = 128
      DataSource = SrcCapacityPlan
      Columns = <
        item
          FieldName = 'EMP_LEVEL_CODE'
          Title.Caption = 'EMP_LEVEL_CODE'
          Width = 64
          CheckBoxField.FieldValues = 'true;false'
        end
        item
          FieldName = 'Period1'
          Title.Caption = 'Period1'
          Width = 64
          ReadOnly = True
          CheckBoxField.FieldValues = 'true;false'
        end
        item
          FieldName = 'Period2'
          Title.Caption = 'Period2'
          Width = 64
          ReadOnly = True
          CheckBoxField.FieldValues = 'true;false'
        end
        item
          FieldName = 'Period3'
          Title.Caption = 'Period3'
          Width = 64
          ReadOnly = True
          CheckBoxField.FieldValues = 'true;false'
        end
        item
          FieldName = 'Period4'
          Title.Caption = 'Period4'
          Width = 64
          ReadOnly = True
          CheckBoxField.FieldValues = 'true;false'
        end
        item
          FieldName = 'Period5'
          Title.Caption = 'Period5'
          Width = 64
          CheckBoxField.FieldValues = 'true;false'
        end
        item
          FieldName = 'HRS_REQ_FUTURE'
          Title.Caption = 'Future Hours'
          Width = 64
          ReadOnly = True
          CheckBoxField.FieldValues = 'true;false'
        end>
      ExplicitWidth = 936
      Marker = 0
    end
    object Panel2: TUniPanel
      Left = 1
      Top = 1
      Width = 932
      Height = 36
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Align = alTop
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 1
      Caption = ''
      ExplicitWidth = 936
      object sbtnBack: TUniSpeedButton
        Left = 47
        Top = 1
        Width = 32
        Height = 32
        Hint = 'Move Back'
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        OnClick = sbtnBackClick
      end
      object sbtnForward: TUniSpeedButton
        Left = 79
        Top = 1
        Width = 32
        Height = 32
        Hint = 'Move Forward'
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        OnClick = sbtnForwardClick
      end
      object bbtnSearch: TUniSpeedButton
        Left = 1
        Top = 1
        Width = 32
        Height = 32
        Hint = 'Search...'
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        OnClick = bbtnSearchClick
      end
      object Bevel1: TUniPanel
        Left = 39
        Top = 1
        Width = 3
        Height = 32
        Hint = ''
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        TabOrder = 5
        Caption = ''
      end
      object Panel3: TUniPanel
        Left = 862
        Top = 1
        Width = 70
        Height = 34
        Hint = ''
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        ShowHint = True
        ParentShowHint = False
        Align = alRight
        Anchors = [akTop, akRight, akBottom]
        TabOrder = 0
        Caption = ''
        ExplicitLeft = 689
        object Shape1: TUniPanel
          Left = 2
          Top = 10
          Width = 13
          Height = 13
          Hint = 'Available'
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          ShowHint = True
          TabOrder = 1
          Caption = ''
        end
        object Shape4: TUniPanel
          Left = 36
          Top = 10
          Width = 12
          Height = 13
          Hint = 'Over-booked'
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          ShowHint = True
          TabOrder = 2
          Caption = ''
        end
        object Shape3: TUniPanel
          Left = 18
          Top = 10
          Width = 13
          Height = 13
          Hint = 'Booked'
          Margins.Left = 4
          Margins.Top = 4
          Margins.Right = 4
          Margins.Bottom = 4
          ShowHint = True
          TabOrder = 3
          Caption = ''
        end
      end
    end
  end
  object pnlCriteria: TUniPanel
    Left = 0
    Top = 0
    Width = 238
    Height = 480
    Hint = ''
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Align = alLeft
    Anchors = [akLeft, akTop, akBottom]
    TabOrder = 1
    Caption = ''
    object Label1: TUniLabel
      Left = 5
      Top = 47
      Width = 66
      Height = 17
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Caption = 'Begin Date'
      TabOrder = 4
    end
    object Label2: TUniLabel
      Left = 5
      Top = 81
      Width = 69
      Height = 17
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Caption = 'View Period'
      TabOrder = 5
    end
    object Panel4: TUniPanel
      Left = 1
      Top = 1
      Width = 236
      Height = 36
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Align = alTop
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 0
      Caption = ''
      object sbtnApply: TUniSpeedButton
        Left = 1
        Top = 1
        Width = 32
        Height = 32
        Hint = 'Apply Criteria'
        Margins.Left = 4
        Margins.Top = 4
        Margins.Right = 4
        Margins.Bottom = 4
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        OnClick = sbtnApplyClick
      end
    end
    object wwDBDateTimePickerStartDate: TUniDBDateTimePicker
      Left = 92
      Top = 42
      Width = 131
      Height = 25
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      DateTime = 42646.000000000000000000
      DateFormat = 'dd/MM/yyyy'
      TimeFormat = 'HH:mm:ss'
      TabOrder = 1
    end
    object wwDBComboBoxViewType: TUniDBComboBox
      Left = 92
      Top = 76
      Width = 131
      Height = 25
      Hint = ''
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Style = csDropDownList
      Items.Strings = (
        'Monthly'#9'M'
        'Weekly'#9'W'
        'Daily'#9'D')
      TabOrder = 2
      OnChange = wwDBComboBoxViewTypeChange
    end
  end
  object SrcCapacityPlan: TDataSource
    DataSet = QryMonthly
    Left = 245
    Top = 97
  end
  object QryMonthly: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      ''
      '       -- period 1'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      '       a.hrs_week_avail * 4 as hrs_avail_1,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                    '
      '       -- period 2'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      '       a.hrs_week_avail * 4 hrs_avail_2,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                              '
      '       -- period 3'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      '       a.hrs_week_avail * 4 hrs_avail_3,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      '       a.hrs_week_avail * 4 hrs_avail_4,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      '       a.hrs_week_avail * 4 hrs_avail_5,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    -1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    -1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      
        '       (select * from v_fore_mfgtype_emp_level where fore_head_i' +
        'd = :fore_head_id) v,'
      
        '       (select * from v_fore_day_labor_week where fore_head_id =' +
        ' :fore_head_id) r, '
      '       v_labor_week_avail a'
      ' where'
      '       v.emp_level_id = a.emp_level_id(+)'
      '   and v.mfgtype = rtrim(a.mfgtype(+))'
      '   and nvl(v.mfgcell_id,0) = NVL(a.mfgcell_id(+),0)'
      '   and v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      '   and NVL(v.mfgcell_id, 0) = NVL( r.mfgcell_id(+), 0)'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.mfgcell_id,'
      '       v.mfgcell,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       a.hrs_week_avail'
      ''
      'order by'
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.emp_level_code'
      '')
    Left = 244
    Top = 134
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDate
        ParamType = ptInput
        Value = Null
      end
      item
        Name = 'fore_head_id'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object QryMonthlyEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryMonthlyPeriod1: TFloatField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period1'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod2: TFloatField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period2'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod3: TFloatField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period3'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod4: TFloatField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period4'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod5: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
    object QryMonthlyHRS_REQ_FUTURE: TFMTBCDField
      DisplayLabel = 'Future Hours'
      DisplayWidth = 17
      FieldName = 'HRS_REQ_FUTURE'
      ReadOnly = True
      DisplayFormat = '########0.00'
    end
    object QryMonthlyMFGTYPE: TStringField
      DisplayLabel = 'Mfg Type'
      DisplayWidth = 9
      FieldName = 'MFGTYPE'
      ReadOnly = True
      Visible = False
      FixedChar = True
      Size = 10
    end
    object QryMonthlyMFGCELL: TStringField
      DisplayLabel = 'Mfg Cell'
      DisplayWidth = 9
      FieldName = 'MFGCELL'
      ReadOnly = True
      Visible = False
      FixedChar = True
      Size = 15
    end
    object QryMonthlyHRS_REQ_1: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_1'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlyHRS_AVAIL_1: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_1'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlyHRS_REQ_2: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_2'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlyHRS_AVAIL_2: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_2'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlyHRS_REQ_3: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_3'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlyHRS_AVAIL_3: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_3'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlyHRS_REQ_4: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_4'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlyHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
    end
    object QryMonthlyHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
    end
    object QryMonthlyHRS_AVAIL_4: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_4'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
    end
    object QryMonthlySETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      DisplayFormat = '########0.00'
    end
    object QryMonthlySETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      DisplayFormat = '########0.00'
    end
    object QryMonthlySETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      DisplayFormat = '########0.00'
    end
    object QryMonthlySETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      DisplayFormat = '########0.00'
    end
    object QryMonthlySETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      DisplayFormat = '########0.00'
    end
    object QryMonthlySETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
    end
    object QryMonthlyMFGCELL_ID: TBCDField
      FieldName = 'MFGCELL_ID'
      Origin = 'MFGCELL_ID'
      Precision = 15
      Size = 0
    end
    object QryMonthlyEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Origin = 'EMP_LEVEL_ID'
      Precision = 15
      Size = 0
    end
  end
  object MainMenu1: TUniMainMenu
    Left = 324
    Top = 97
    object File1: TUniMenuItem
      Caption = 'File'
      object Close1: TUniMenuItem
        Caption = 'Exit'
        OnClick = Close1Click
      end
    end
    object Help1: TUniMenuItem
      Caption = 'Help'
      object Contents1: TUniMenuItem
        Caption = 'Contents'
        OnClick = Contents1Click
      end
      object About1: TUniMenuItem
        Caption = 'About'
        OnClick = About1Click
      end
    end
  end
  object IQAbout1: TIQWebAbout
    ModuleName = 'FrmForeLabCapWhiteboard'
    Version = 'DEV.176554'
    ChangeListNumber = '$Change: 191836 $'
    BuildVersion = '176554'
    Left = 357
    Top = 98
  end
  object QryWeekly: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      ''
      '       -- period 1'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      '       a.hrs_week_avail as hrs_avail_1,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                  '
      '       -- period 2'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      '       a.hrs_week_avail as hrs_avail_2,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                                  '
      '       -- period 3'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      '       a.hrs_week_avail as hrs_avail_3,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      '       a.hrs_week_avail as hrs_avail_4,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      '       a.hrs_week_avail as hrs_avail_5,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    -1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    -1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      
        '       (select * from v_fore_mfgtype_emp_level where fore_head_i' +
        'd = :fore_head_id) v,'
      
        '       (select * from v_fore_day_labor_week where fore_head_id =' +
        ' :fore_head_id) r, '
      '       v_labor_week_avail a'
      ' where'
      '       v.emp_level_id = a.emp_level_id(+)'
      '   and v.mfgtype = rtrim(a.mfgtype(+))'
      '   and nvl(v.mfgcell_id,0) = NVL(a.mfgcell_id(+),0)'
      '   '
      '   and v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      '   and NVL(v.mfgcell_id, 0) = NVL( r.mfgcell_id(+), 0)'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.mfgcell_id,'
      '       v.mfgcell,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       a.hrs_week_avail'
      ''
      'order by'
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.emp_level_code'
      '')
    Left = 276
    Top = 134
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDate
        ParamType = ptInput
        Value = Null
      end
      item
        Name = 'fore_head_id'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object QryWeeklyMFGTYPE: TStringField
      FieldName = 'MFGTYPE'
      Origin = 'MFGTYPE'
      Size = 10
    end
    object QryWeeklyMFGCELL: TStringField
      FieldName = 'MFGCELL'
      Origin = 'MFGCELL'
      Size = 15
    end
    object QryWeeklyMFGCELL_ID: TBCDField
      FieldName = 'MFGCELL_ID'
      Origin = 'MFGCELL_ID'
      Precision = 15
      Size = 0
    end
    object QryWeeklyEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Origin = 'EMP_LEVEL_ID'
      Precision = 15
      Size = 0
    end
    object QryWeeklyEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Origin = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryWeeklyHRS_REQ_1: TFMTBCDField
      FieldName = 'HRS_REQ_1'
      Origin = 'HRS_REQ_1'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_1: TFMTBCDField
      FieldName = 'HRS_AVAIL_1'
      Origin = 'HRS_AVAIL_1'
      Precision = 38
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      Origin = 'SETUP_HRS_REQ_1'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_REQ_2: TFMTBCDField
      FieldName = 'HRS_REQ_2'
      Origin = 'HRS_REQ_2'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_2: TFMTBCDField
      FieldName = 'HRS_AVAIL_2'
      Origin = 'HRS_AVAIL_2'
      Precision = 38
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      Origin = 'SETUP_HRS_REQ_2'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_REQ_3: TFMTBCDField
      FieldName = 'HRS_REQ_3'
      Origin = 'HRS_REQ_3'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_3: TFMTBCDField
      FieldName = 'HRS_AVAIL_3'
      Origin = 'HRS_AVAIL_3'
      Precision = 38
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      Origin = 'SETUP_HRS_REQ_3'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_REQ_4: TFMTBCDField
      FieldName = 'HRS_REQ_4'
      Origin = 'HRS_REQ_4'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_4: TFMTBCDField
      FieldName = 'HRS_AVAIL_4'
      Origin = 'HRS_AVAIL_4'
      Precision = 38
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      Origin = 'SETUP_HRS_REQ_4'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      Origin = 'HRS_REQ_5'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      Origin = 'HRS_AVAIL_5'
      Precision = 38
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      Origin = 'SETUP_HRS_REQ_5'
      Precision = 38
      Size = 38
    end
    object QryWeeklyHRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'HRS_REQ_FUTURE'
      Origin = 'HRS_REQ_FUTURE'
      Precision = 38
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      Origin = 'SETUP_HRS_REQ_FUTURE'
      Precision = 38
      Size = 38
    end
    object QryWeeklyPeriod1: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period1'
      Calculated = True
    end
    object QryWeeklyPeriod2: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period2'
      Calculated = True
    end
    object QryWeeklyPeriod3: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period3'
      Calculated = True
    end
    object QryWeeklyPeriod4: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period4'
      Calculated = True
    end
    object QryWeeklyPeriod5: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
  end
  object QryDaily: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       '
      '       -- period 1'
      '       sum( decode( trunc(r.prod_date) - to_date(:start_date),'
      '                    0, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      
        '       sum( decode( trunc(r.prod_date) - to_date(:start_date),  ' +
        '     '
      '                    0, a.hrs_avail,'
      '                    0)) as hrs_avail_1,'
      '       sum( decode( trunc(r.prod_date) - to_date(:start_date),'
      '                    0, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                                   '
      '       -- period 2'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 1, '#39'N'#39'),'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 1, '#39'N'#39'),       '
      '                    1, a.hrs_avail,'
      '                    0)) as hrs_avail_2,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 1, '#39'N'#39'),'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                                  '
      '       -- period 3'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 2, '#39'N'#39'),'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 2, '#39'N'#39'),       '
      '                    2, a.hrs_avail,'
      '                    0)) as hrs_avail_3,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 2, '#39'N'#39'),'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 3, '#39'N'#39'),'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 3, '#39'N'#39'),       '
      '                    3, a.hrs_avail,'
      '                    0)) as hrs_avail_4,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 3, '#39'N'#39'),'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 4, '#39'N'#39'),'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 4, '#39'N'#39'),       '
      '                    4, a.hrs_avail,'
      '                    0)) as hrs_avail_5,'
      
        '       sum( decode( trunc(r.prod_date) - iqcalendar.add_days(:st' +
        'art_date, 4, '#39'N'#39'),'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( Sign( trunc(r.prod_date) - iqcalendar.add_da' +
        'ys(:start_date, 4, '#39'N'#39') - 4 ),'
      '                    +1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( Sign( trunc(r.prod_date) - iqcalendar.add_da' +
        'ys(:start_date, 4, '#39'N'#39') - 4 ),'
      '                    +1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      
        '       (select * from v_fore_mfgtype_emp_level where fore_head_i' +
        'd = :fore_head_id) v,'
      
        '       (select * from v_fore_day_labor_day where fore_head_id = ' +
        ':fore_head_id) r, '
      '       v_labor_day_avail a'
      ' where'
      '       v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      '   and NVL(v.mfgcell_id, 0) = NVL( r.mfgcell_id(+), 0)'
      ''
      '   and r.emp_level_id = a.emp_level_id(+)'
      '   and r.mfgtype = rtrim(a.mfgtype(+))'
      '   and nvl(r.mfgcell_id,0) = NVL(a.mfgcell_id(+),0)'
      '   '
      
        '   and trunc(r.prod_date) = to_date(:start_date) + a.week_day(+)' +
        ' - 1'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.mfgcell_id,'
      '       v.mfgcell,'
      '       v.emp_level_id,'
      '       v.emp_level_code'
      '       '
      'order by'
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.emp_level_code')
    Left = 316
    Top = 134
    ParamData = <
      item
        Name = 'START_DATE'
        DataType = ftDate
        ParamType = ptInput
        Value = Null
      end
      item
        Name = 'FORE_HEAD_ID'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object QryDailyMFGTYPE: TStringField
      FieldName = 'MFGTYPE'
      Origin = 'MFGTYPE'
      Size = 10
    end
    object QryDailyMFGCELL: TStringField
      FieldName = 'MFGCELL'
      Origin = 'MFGCELL'
      Size = 15
    end
    object QryDailyMFGCELL_ID: TBCDField
      FieldName = 'MFGCELL_ID'
      Origin = 'MFGCELL_ID'
      Precision = 15
      Size = 0
    end
    object QryDailyEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Origin = 'EMP_LEVEL_ID'
      Precision = 15
      Size = 0
    end
    object QryDailyEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Origin = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryDailyHRS_REQ_1: TFMTBCDField
      FieldName = 'HRS_REQ_1'
      Origin = 'HRS_REQ_1'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_AVAIL_1: TFMTBCDField
      FieldName = 'HRS_AVAIL_1'
      Origin = 'HRS_AVAIL_1'
      Precision = 38
      Size = 38
    end
    object QryDailySETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      Origin = 'SETUP_HRS_REQ_1'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_REQ_2: TFMTBCDField
      FieldName = 'HRS_REQ_2'
      Origin = 'HRS_REQ_2'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_AVAIL_2: TFMTBCDField
      FieldName = 'HRS_AVAIL_2'
      Origin = 'HRS_AVAIL_2'
      Precision = 38
      Size = 38
    end
    object QryDailySETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      Origin = 'SETUP_HRS_REQ_2'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_REQ_3: TFMTBCDField
      FieldName = 'HRS_REQ_3'
      Origin = 'HRS_REQ_3'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_AVAIL_3: TFMTBCDField
      FieldName = 'HRS_AVAIL_3'
      Origin = 'HRS_AVAIL_3'
      Precision = 38
      Size = 38
    end
    object QryDailySETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      Origin = 'SETUP_HRS_REQ_3'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_REQ_4: TFMTBCDField
      FieldName = 'HRS_REQ_4'
      Origin = 'HRS_REQ_4'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_AVAIL_4: TFMTBCDField
      FieldName = 'HRS_AVAIL_4'
      Origin = 'HRS_AVAIL_4'
      Precision = 38
      Size = 38
    end
    object QryDailySETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      Origin = 'SETUP_HRS_REQ_4'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      Origin = 'HRS_REQ_5'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      Origin = 'HRS_AVAIL_5'
      Precision = 38
      Size = 38
    end
    object QryDailySETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      Origin = 'SETUP_HRS_REQ_5'
      Precision = 38
      Size = 38
    end
    object QryDailyHRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'HRS_REQ_FUTURE'
      Origin = 'HRS_REQ_FUTURE'
      Precision = 38
      Size = 38
    end
    object QryDailySETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      Origin = 'SETUP_HRS_REQ_FUTURE'
      Precision = 38
      Size = 38
    end
    object QryDailyPeriod1: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period1'
      Calculated = True
    end
    object QryDailyPeriod2: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period2'
      Calculated = True
    end
    object QryDailyPeriod3: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period3'
      Calculated = True
    end
    object QryDailyPeriod4: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period4'
      Calculated = True
    end
    object QryDailyPeriod5: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
  end
  object PkType: TIQWebHpick
    ConnectionName = 'IQFD'
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.emp_level_code,'
      '       v.emp_level_id,'
      '       v.mfgcell_id'
      '  from '
      '       v_fore_mfgtype_emp_level v'
      '')
    Title = 'Select from Pick List'
    Touchscreen = False
    ForceShowWaitForPrompt = False
    Left = 389
    Top = 97
    object PkTypeEMP_LEVEL_CODE: TStringField
      DisplayLabel = 'Employee Level Code'
      FieldName = 'EMP_LEVEL_CODE'
      Origin = 'IQ.V_MFGTYPE_EMP_LEVEL.EMP_LEVEL_CODE'
      Size = 10
    end
    object PkTypeMFGTYPE: TStringField
      DisplayLabel = 'Mfg Type'
      FieldName = 'MFGTYPE'
      Origin = 'IQ.V_WORK_CENTER_TYPE.MFGTYPE'
      FixedChar = True
      Size = 10
    end
    object PkTypeMFGCELL: TStringField
      DisplayLabel = 'Mfg Cell'
      FieldName = 'MFGCELL'
      Origin = 'IQ.V_WORK_CENTER_TYPE.MFGCELL'
      FixedChar = True
      Size = 15
    end
    object PkTypeMFGCELL_ID: TBCDField
      FieldName = 'MFGCELL_ID'
      Origin = 'IQ.V_WORK_CENTER_TYPE.MFGCELL_ID'
      Visible = False
      Size = 0
    end
    object PkTypeEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Origin = 'IQ.V_MFGTYPE_EMP_LEVEL.EMP_LEVEL_ID'
      Visible = False
      Size = 0
    end
  end
end
