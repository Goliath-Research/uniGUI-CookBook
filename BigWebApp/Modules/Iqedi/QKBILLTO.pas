...
type
  TFrmQkBillTo = class(TQuickAddBase)
    Table1ID: TBCDField;
...
function DoQuickAppendBillTo( var AResultValue : Variant; AID:Real ): TModalResult;
...
function DoQuickAppendBillTo( var AResultValue : Variant; AID:Real ): TModalResult;
var
  LFrmQkBillTo : TFrmQkBillTo;
begin
  LFrmQkBillTo := TFrmQkBillTo.Create(UniGUIApplication.UniApplication);
  with LFrmQkBillTo do
  begin
    ID :=AID;
    Result:= ShowModal;
    if Result = mrOK then
...
procedure TFrmQkBillTo.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TFrmQkBillTo.SetID(const Value: Real);
begin
  FID := Value;
end;
