object CostElm: TCostElm
  Left = 316
  Top = 191
  HelpContext = 999213
  ClientHeight = 521
  ClientWidth = 963
  Caption = 'Cost Elements'
  OnShow = FormShow
  OldCreateOrder = True
  OnClose = FormClose
  OnActivate = FormActivate
  MonitoredKeys.Keys = <>
  ActiveControl = Panel1
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Bevel1: TUniPanel
    Left = 0
    Top = 27
    Width = 963
    Height = 3
    Hint = ''
    Align = alTop
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 2
    Caption = ''
  end
  object Panel1: TUniPanel
    Left = 0
    Top = 0
    Width = 963
    Height = 27
    Hint = ''
    Align = alTop
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 0
    Caption = ''
    object sbtnForm: TUniSpeedButton
      Left = 1
      Top = 1
      Width = 25
      Height = 25
      Hint = 'View as Form'
      Visible = False
      ShowHint = True
      ParentShowHint = False
      Glyph.Data = {
        
        1412100E0C0A1D1D1D1D372E25211E050404030303031D1D1D1D}
      Caption = ''
      ParentColor = False
      Color = clWindow
      OnClick = sbtnFormClick
    end
    object NavMain: TUniDBNavigator
      Left = 738
      Top = 1
      Width = 225
      Height = 25
      Hint = ''
      DataSource = DataSource1
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbInsert, nbDelete, nbPost, nbCancel, nbRefresh]
      Align = alRight
      Anchors = [akTop, akRight, akBottom]
      TabOrder = 0
    end
  end
  object Notebook1: TUniPageControl
    Left = 0
    Top = 30
    Width = 963
    Height = 491
    Hint = ''
    TabBarVisible = True
    Align = alClient
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 1
    object TUniTabSheet
      Hint = ''
      Caption = 'Form'
      object Label2: TUniLabel
        Left = 13
        Top = 25
        Width = 57
        Height = 13
        Hint = ''
        AutoSize = False
        Caption = 'Description'
        TabOrder = 3
      end
      object Label3: TUniLabel
        Left = 13
        Top = 52
        Width = 28
        Height = 13
        Hint = ''
        AutoSize = False
        Caption = 'Type'
        TabOrder = 4
      end
      object Label4: TUniLabel
        Left = 13
        Top = 80
        Width = 61
        Height = 13
        Hint = ''
        AutoSize = False
        Caption = 'GL Account'
        TabOrder = 5
      end
      object sbtnGLAcct: TUniSpeedButton
        Left = 332
        Top = 74
        Width = 23
        Height = 25
        Hint = ''
        Caption = '...'
        ParentFont = False
        Font.Color = clBlack
        Font.Height = -13
        Font.Style = [fsBold]
        ParentColor = False
        Color = clWindow
      end
      object EditELEM_DESCRIP: TUniDBEdit
        Left = 79
        Top = 21
        Width = 165
        Height = 21
        Hint = ''
        DataField = 'ELEM_DESCRIP'
        DataSource = DataSource1
        TabOrder = 0
      end
      object DBEdit1: TUniDBEdit
        Left = 79
        Top = 48
        Width = 165
        Height = 21
        Hint = ''
        DataField = 'ELEM_TYPE'
        DataSource = DataSource1
        TabOrder = 1
      end
      object DBEdit2: TUniDBEdit
        Left = 79
        Top = 76
        Width = 249
        Height = 21
        Hint = ''
        DataField = 'GLAcct'
        DataSource = DataSource1
        TabOrder = 2
      end
    end
    object TUniTabSheet
      Hint = ''
      Caption = 'Grid'
      inline GridElements: TIQUniGridControl
        Left = 0
        Top = 0
        Width = 955
        Height = 463
        Layout = 'border'
        ParentAlignmentControl = False
        AlignmentControl = uniAlignmentClient
        Align = alClient
        Anchors = [akLeft, akTop, akRight, akBottom]
        TabOrder = 0
        ExplicitWidth = 955
        ExplicitHeight = 463
        inherited PageControl: TUniPageControl
          Width = 955
          Height = 434
          ExplicitWidth = 955
          ExplicitHeight = 434
          inherited GridTab: TUniTabSheet
            ExplicitLeft = 4
            ExplicitTop = 24
            ExplicitWidth = 947
            ExplicitHeight = 406
            inherited DBGrid: TUniDBGrid
              Width = 738
              Height = 377
              Columns = <>
            end
            inherited GridEditorsPanel: TUniHiddenPanel
              Left = 738
              Height = 377
              ExplicitLeft = 738
              ExplicitHeight = 377
            end
            inherited GridToolBar: TUniToolBar
              Width = 947
              ExplicitWidth = 947
            end
          end
          inherited FormTab: TUniTabSheet
            ExplicitLeft = 4
            ExplicitTop = 24
            ExplicitWidth = 947
            ExplicitHeight = 406
          end
        end
        inherited TopToolBar: TUniToolBar
          Width = 955
          ExplicitWidth = 955
          inherited UniToolButton1: TUniToolButton
            ExplicitLeft = 2
            ExplicitTop = 1
          end
        end
        inherited ImgList: TUniNativeImageList
          Images = {
            
            426082}
        end
      end
      object wwDBComboBox1: TUniDBComboBox
        Left = 252
        Top = 100
        Width = 130
        Height = 21
        Hint = ''
        DataField = 'ELEM_TYPE'
        DataSource = DataSource1
        Style = csDropDownList
        Items.Strings = (
          'Labor'#9'LABOR'
          'Outsource'#9'OUTSOURCE'
          'Overhead'#9'OVERHEAD'
          'Fixed Overhead'#9'OVERHEADF'
          'Landed Cost'#9'LANDEDCOST')
        TabOrder = 1
      end
      object wwDBComboDlg1: TUniEdit
        Left = 388
        Top = 100
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 2
        ReadOnly = True
        OnKeyDown = ClearComboDlg
      end
      object wwDBComboDlg2: TUniEdit
        Left = 388
        Top = 127
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 3
        ReadOnly = True
        OnKeyDown = ClearComboDlg
      end
      object wwDBComboDlg3: TUniEdit
        Left = 388
        Top = 154
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 4
        ReadOnly = True
        OnKeyDown = ClearComboDlg
      end
      object wwDBComboDlg4: TUniEdit
        Left = 388
        Top = 181
        Width = 121
        Height = 21
        Hint = ''
        Text = ''
        TabOrder = 5
        ReadOnly = True
        OnKeyDown = ClearComboDlg
      end
    end
  end
  object DataSource1: TDataSource
    DataSet = qryElements
    Left = 8
    Top = 155
  end
  object PKAcct: TIQWebHpick
    ConnectionName = 'IQFD'
    SQL.Strings = (
      'select id,'
      '       acct,'
      '       descrip,'
      '       type,'
      '       eplant_id'
      '  from glacct  '
      'where NVL(PK_HIDE, '#39'N'#39') = '#39'N'#39'    '
      '  and misc.eplant_filter(eplant_id) = 1')
    Title = 'Select from Pick List'
    Touchscreen = False
    ForceShowWaitForPrompt = False
    Left = 52
    Top = 123
    object PKAcctID: TBCDField
      FieldName = 'ID'
      Visible = False
      Size = 0
    end
    object PKAcctACCT: TStringField
      DisplayLabel = 'Account'
      FieldName = 'ACCT'
      Size = 50
    end
    object PKAcctDESCRIP: TStringField
      DisplayLabel = 'Description'
      FieldName = 'DESCRIP'
      Size = 50
    end
    object PKAcctTYPE: TStringField
      DisplayLabel = 'Type'
      FieldName = 'TYPE'
      Size = 25
    end
    object PKAcctEPLANT_ID: TBCDField
      DisplayLabel = 'EPlant ID'
      FieldName = 'EPLANT_ID'
      Size = 0
    end
  end
  object SR: TIQWebSecurityRegister
    SecurityItems.Strings = (
      'NavMain'
      'GridElements'
      'sbtnGLAcct'
      'TblElements'
      'TblElementsGLACCT_ID'
      'TblElementsACCT_ID_PROD_VARIANCE'
      'TblElementsACCT_ID_RECV_DISPO_INV'
      'TblElementsACCT_ID_ACCRUED_RECV')
    SecurityCode = 'COSTELM'
    Left = 96
  end
  object wwQryGLAcctLookup: TFDQuery
    Tag = 1
    Connection = UniMainModule.FDConnection1
    UpdateOptions.AssignedValues = [uvEDelete, uvEInsert, uvEUpdate]
    UpdateOptions.EnableDelete = False
    UpdateOptions.EnableInsert = False
    UpdateOptions.EnableUpdate = False
    SQL.Strings = (
      'select * from glacct'
      'where NVL(PK_HIDE, '#39'N'#39') = '#39'N'#39
      'order by acct')
    Left = 144
    Top = 139
    object wwQryGLAcctLookupACCT: TStringField
      DisplayLabel = 'Acct#'
      DisplayWidth = 25
      FieldName = 'ACCT'
      Origin = 'GLACCT.ACCT'
      Size = 50
    end
    object wwQryGLAcctLookupDESCRIP: TStringField
      DisplayLabel = 'Description'
      DisplayWidth = 25
      FieldName = 'DESCRIP'
      Origin = 'GLACCT.DESCRIP'
      Size = 50
    end
    object wwQryGLAcctLookupID: TBCDField
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'GLACCT.ID'
      Visible = False
      Size = 0
    end
  end
  object qryElements: TFDQuery
    BeforePost = AssignIDBeforePost
    OnCalcFields = qryElementsCalcFields
    IndexFieldNames = 'ID'
    Connection = UniMainModule.FDConnection1
    UpdateOptions.AssignedValues = [uvUpdateMode]
    UpdateOptions.UpdateTableName = 'ELEMENTS'
    UpdateOptions.KeyFields = 'ID'
    SQL.Strings = (
      'Select * from ELEMENTS')
    Left = 216
    Top = 262
    object qryElementsELEM_DESCRIP: TStringField
      DisplayLabel = 'Element Description'
      DisplayWidth = 22
      FieldName = 'ELEM_DESCRIP'
      Required = True
      Size = 30
    end
    object qryElementsELEM_TYPE: TStringField
      DisplayLabel = 'Type'
      DisplayWidth = 15
      FieldName = 'ELEM_TYPE'
      Size = 10
    end
    object qryElementsGLAcct: TStringField
      DisplayLabel = 'COGS ~GL Account #'
      DisplayWidth = 22
      FieldKind = fkCalculated
      FieldName = 'GLAcct'
      LookupKeyFields = 'ID'
      LookupResultField = 'ACCT'
      KeyFields = 'GLACCT_ID'
      Size = 50
      Calculated = True
    end
    object qryElementsAcct_Prod_Var: TStringField
      DisplayLabel = 'Production Variance ~GL Account #'
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Acct_Prod_Var'
      LookupKeyFields = 'ID'
      LookupResultField = 'ACCT'
      KeyFields = 'ACCT_ID_PROD_VARIANCE'
      Size = 50
      Calculated = True
    end
    object qryElementsAcct_Recv_Dispo: TStringField
      DisplayLabel = 'Receive / Dispo ~GL Account #'
      DisplayWidth = 24
      FieldKind = fkCalculated
      FieldName = 'Acct_Recv_Dispo'
      LookupKeyFields = 'ID'
      LookupResultField = 'ACCT'
      KeyFields = 'ACCT_ID_RECV_DISPO_INV'
      Size = 50
      Calculated = True
    end
    object qryElementsAccr_receipts: TStringField
      DisplayLabel = 'Accrued Receipts ~GL Account #'
      DisplayWidth = 25
      FieldKind = fkCalculated
      FieldName = 'Accr_receipts'
      LookupKeyFields = 'ID'
      LookupResultField = 'ACCT'
      KeyFields = 'ACCT_ID_ACCRUED_RECV'
      Size = 50
      Calculated = True
    end
    object qryElementsDEL_ON_ROLL_OVER: TStringField
      DisplayLabel = 'Remove During ~Roll Over'
      DisplayWidth = 14
      FieldName = 'DEL_ON_ROLL_OVER'
      FixedChar = True
      Size = 1
    end
    object qryElementsAcct_Prod_Var2: TStringField
      DisplayLabel = 'Production Variance GL Account #2'
      DisplayWidth = 20
      FieldKind = fkCalculated
      FieldName = 'Acct_Prod_Var2'
      LookupKeyFields = 'ID'
      LookupResultField = 'ACCT'
      KeyFields = 'ACCT_ID_PROD_VARIANCE2'
      Visible = False
      Calculated = True
    end
    object qryElementsID: TBCDField
      DisplayWidth = 10
      FieldName = 'ID'
      Visible = False
      Size = 0
    end
    object qryElementsGLACCT_ID: TBCDField
      DisplayWidth = 10
      FieldName = 'GLACCT_ID'
      Visible = False
      Size = 0
    end
    object qryElementsACCT_ID_PROD_VARIANCE: TBCDField
      DisplayWidth = 10
      FieldName = 'ACCT_ID_PROD_VARIANCE'
      Visible = False
      Size = 0
    end
    object qryElementsACCT_ID_RECV_DISPO_INV: TBCDField
      DisplayWidth = 10
      FieldName = 'ACCT_ID_RECV_DISPO_INV'
      Visible = False
      Size = 0
    end
    object qryElementsACCT_ID_ACCRUED_RECV: TBCDField
      DisplayWidth = 10
      FieldName = 'ACCT_ID_ACCRUED_RECV'
      Visible = False
      Size = 0
    end
    object qryElementsACCT_ID_PROD_VARIANCE2: TBCDField
      DisplayWidth = 10
      FieldName = 'ACCT_ID_PROD_VARIANCE2'
      Visible = False
      Size = 0
    end
  end
end
