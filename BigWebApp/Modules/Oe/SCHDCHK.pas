...
begin
  ...
  begin
    ...
    //removed the not in the if statement to test it. Worked properly
    if not (QryScheduled.Eof and QryScheduled.Bof) then
    begin
       if not (ShowModal = mrOK) then
          raise TIQNotAuthorizedException.Create;
    end
    else
       // Don't want to show the form
       LFrmOrdScheduledException.free;
  end;
end;
