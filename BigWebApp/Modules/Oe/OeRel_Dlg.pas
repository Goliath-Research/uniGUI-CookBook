...
{$ENDREGION}
  ...
  public
    { Public declarations }
    property RelInputCargo: TRelInputCargo write SetRelInputCargo;
  end;
...
function DoReleasesDialog(var ARelInputCargo: TRelInputCargo ): Boolean;
var
  LFrmNewOeReleasesDialog : TFrmNewOeReleasesDialog;
begin
  Result:= False;
  LFrmNewOeReleasesDialog :=  TFrmNewOeReleasesDialog.Create(UniGUIApplication.UniApplication);
  LFrmNewOeReleasesDialog.RelInputCargo := ARelInputCargo;
  Result:= LFrmNewOeReleasesDialog.ShowModal = mrOK;
  if Result then
    LFrmNewOeReleasesDialog.SaveTo( ARelInputCargo );
end;
...
begin
  ...
  begin
    ...
    dtpStartDate.Datetime    := IfThen( Start_Date = 0, Date, Start_Date + LeadDays);
...
procedure TFrmNewOeReleasesDialog.SetRelInputCargo(const Value: TRelInputCargo);
begin
  FRelInputCargo := Value;
end;

procedure TFrmNewOeReleasesDialog.UniFormCreate(Sender: TObject);
var
...
{ TODO -oGPatil -cWebConvert : Invalid Datatype for Scalar value Error
