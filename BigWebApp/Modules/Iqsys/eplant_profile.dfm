object FrmEPlantProfile: TFrmEPlantProfile
  Left = 240
  Top = 191
  ClientHeight = 179
  ClientWidth = 408
  Caption = 'EPlant Profile'
  OnShow = UniFormShow
  OldCreateOrder = False
  OnClose = FormClose
  MonitoredKeys.Keys = <>
  PixelsPerInch = 96
  TextHeight = 13
  object Bevel1: TUniPanel
    Left = 0
    Top = 0
    Width = 408
    Height = 3
    Hint = ''
    Align = alTop
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 1
    Caption = ''
    ExplicitTop = 28
  end
  inline wwDBGrid1: TIQUniGridControl
    Left = 0
    Top = 3
    Width = 408
    Height = 157
    Layout = 'border'
    ParentAlignmentControl = False
    AlignmentControl = uniAlignmentClient
    Align = alClient
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 0
    ExplicitTop = 31
    ExplicitWidth = 408
    ExplicitHeight = 129
    inherited PageControl: TUniPageControl
      Width = 408
      Height = 128
      ExplicitWidth = 408
      ExplicitHeight = 100
      inherited GridTab: TUniTabSheet
        ExplicitLeft = 4
        ExplicitTop = 24
        ExplicitWidth = 400
        ExplicitHeight = 72
        inherited DBGrid: TUniDBGrid
          Width = 191
          Height = 71
          Columns = <>
        end
        inherited GridEditorsPanel: TUniHiddenPanel
          Left = 191
          Height = 71
          ExplicitLeft = 191
          ExplicitHeight = 43
        end
        inherited GridToolBar: TUniToolBar
          Width = 400
          ExplicitWidth = 400
        end
      end
      inherited FormTab: TUniTabSheet
        ExplicitLeft = 4
        ExplicitTop = 24
        ExplicitWidth = 400
        ExplicitHeight = 72
      end
    end
    inherited TopToolBar: TUniToolBar
      Width = 408
      ExplicitTop = -6
      ExplicitWidth = 408
    end
    inherited ImgList: TUniNativeImageList
      Images = {
        
        426082}
    end
  end
  object StatusBar1: TUniStatusBar
    Left = 0
    Top = 160
    Width = 408
    Height = 19
    Hint = ''
    Panels = <
      item
        Text = ' Module:'
        Width = 200
      end
      item
        Text = ' Topic:'
        Width = 50
      end>
    SizeGrip = False
    Align = alBottom
    Anchors = [akLeft, akRight, akBottom]
    ParentColor = False
    Color = clWindow
  end
  object wwDBLookupComboEplant: TUniDBLookupComboBox
    Left = 160
    Top = 111
    Width = 121
    Height = 21
    Hint = ''
    ListFieldIndex = 0
    TabOrder = 2
    Color = clWindow
  end
  object SrcEplant_Profile: TDataSource
    DataSet = QryEplant_Profile
    Left = 97
    Top = 62
  end
  object QryEplant_Profile: TFDQuery
    BeforeOpen = QryEplant_ProfileBeforeOpen
    BeforePost = QryEplant_ProfileBeforePost
    OnCalcFields = QryEplant_ProfileCalcFields
    OnNewRecord = QryEplant_ProfileNewRecord
    Connection = UniMainModule.FDConnection1
    UpdateOptions.UpdateTableName = 'EPLANT_PROFILE'
    UpdateOptions.KeyFields = 'ID'
    UpdateObject = UpdateSQLEplant_Profile
    SQL.Strings = (
      'select id,'
      '       eplant_id,'
      '       module,'
      '       topic,'
      '       value1,'
      '       value2'
      '  from eplant_profile'
      ' where '
      '       module = :module'
      '   and topic  = :topic'
      '   and misc.eplant_filter( eplant_id ) = 1'
      ' order by   '
      '       id'
      ''
      ''
      ' '
      ' '
      ' '
      ' ')
    Left = 66
    Top = 117
    ParamData = <
      item
        Name = 'module'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Name = 'topic'
        DataType = ftString
        ParamType = ptInput
      end>
    object QryEplant_ProfileEPlantName: TStringField
      DisplayLabel = 'EPlant'
      DisplayWidth = 15
      FieldKind = fkCalculated
      FieldName = 'EPlantName'
      Size = 30
      Calculated = True
    end
    object QryEplant_ProfileVALUE1: TStringField
      DisplayLabel = 'Value 1'
      DisplayWidth = 21
      FieldName = 'VALUE1'
      Origin = 'IQ.EPLANT_PROFILE.VALUE1'
      Size = 250
    end
    object QryEplant_ProfileVALUE2: TStringField
      DisplayLabel = 'Value 2'
      DisplayWidth = 25
      FieldName = 'VALUE2'
      Origin = 'IQ.EPLANT_PROFILE.VALUE2'
      Size = 250
    end
    object QryEplant_ProfileEPLANT_ID: TBCDField
      DisplayLabel = 'EPlant'
      DisplayWidth = 14
      FieldName = 'EPLANT_ID'
      Origin = 'IQ.EPLANT_PROFILE.EPLANT_ID'
      Visible = False
      Size = 0
    end
    object QryEplant_ProfileID: TBCDField
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'IQ.EPLANT_PROFILE.ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      Size = 0
    end
    object QryEplant_ProfileMODULE: TStringField
      DisplayWidth = 25
      FieldName = 'MODULE'
      Origin = 'IQ.EPLANT_PROFILE.MODULE'
      Visible = False
      Size = 25
    end
    object QryEplant_ProfileTOPIC: TStringField
      DisplayWidth = 25
      FieldName = 'TOPIC'
      Origin = 'IQ.EPLANT_PROFILE.TOPIC'
      Visible = False
      Size = 25
    end
  end
  object wwQueryEplant: TFDQuery
    BeforeOpen = wwQueryEplantBeforeOpen
    Filtered = True
    Connection = UniMainModule.FDConnection1
    UpdateOptions.AssignedValues = [uvEDelete, uvEInsert, uvEUpdate]
    UpdateOptions.EnableDelete = False
    UpdateOptions.EnableInsert = False
    UpdateOptions.EnableUpdate = False
    SQL.Strings = (
      'select id, name from eplant'
      'order by name')
    Left = 327
    Top = 77
    object wwQueryEplantNAME: TStringField
      DisplayLabel = 'EPlant Name'
      DisplayWidth = 30
      FieldName = 'NAME'
      Origin = 'IQ.EPLANT.NAME'
      Size = 30
    end
    object wwQueryEplantID: TBCDField
      DisplayLabel = 'EPlant ID'
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'IQ.EPLANT.ID'
      Size = 0
    end
  end
  object UpdateSQLEplant_Profile: TFDUpdateSQL
    Connection = UniMainModule.FDConnection1
    InsertSQL.Strings = (
      'insert into eplant_profile'
      '  (ID, EPLANT_ID, MODULE, TOPIC, VALUE1, VALUE2)'
      'values'
      '  (:ID, :EPLANT_ID, :MODULE, :TOPIC, :VALUE1, :VALUE2)')
    ModifySQL.Strings = (
      'update eplant_profile'
      'set'
      '  ID = :ID,'
      '  EPLANT_ID = :EPLANT_ID,'
      '  MODULE = :MODULE,'
      '  TOPIC = :TOPIC,'
      '  VALUE1 = :VALUE1,'
      '  VALUE2 = :VALUE2'
      'where'
      '  ID = :OLD_ID')
    DeleteSQL.Strings = (
      'delete from eplant_profile'
      'where'
      '  ID = :OLD_ID')
    Left = 26
    Top = 106
  end
end
