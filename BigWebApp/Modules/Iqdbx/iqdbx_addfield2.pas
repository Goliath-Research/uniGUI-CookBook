...
function AddField(ACurrentTableName: string): Boolean;
var
  FrmIQDBXAddField2 : TFrmIQDBXAddField2;
begin
  FrmIQDBXAddField2 := TFrmIQDBXAddField2.Create(uniGUIApplication.UniApplication);
   with FrmIQDBXAddField2 do
    begin
      FCurrentTableName := ACurrentTableName;
...
begin
  S := cmbType.Text;
  S := StringReplace(S, '%n', IntToStr(udLength.Value)   , [rfReplaceAll]); // length
  S := StringReplace(S, '%s', IntToStr(udLength.Value)   , [rfReplaceAll]); // scale
  S := StringReplace(S, '%p', IntToStr(udPrecision.Value), [rfReplaceAll]); // precision
  Result := S;
end;
