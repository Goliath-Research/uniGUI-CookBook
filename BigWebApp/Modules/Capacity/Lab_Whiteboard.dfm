object FrmLabWhiteboard: TFrmLabWhiteboard
  Left = 96
  Top = 203
  HelpContext = 1011963
  ClientHeight = 382
  ClientWidth = 884
  Caption = 'Labor Whiteboard Capacity Plan'
  OnShow = FormShow
  OldCreateOrder = False
  OnClose = FormClose
  OnActivate = FormActivate
  Menu = MainMenu1
  MonitoredKeys.Keys = <>
  OnCreate = UniFormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter1: TUniSplitter
    Left = 181
    Top = 0
    Width = 6
    Height = 382
    Hint = ''
    Align = alLeft
    ParentColor = False
    Color = clBtnFace
  end
  object Panel1: TUniPanel
    Left = 187
    Top = 0
    Width = 697
    Height = 382
    Hint = ''
    Align = alClient
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 0
    Caption = ''
    inline wwDBGridPlan: TIQUniGridControl
      Left = 1
      Top = 30
      Width = 695
      Height = 351
      Layout = 'border'
      ParentAlignmentControl = False
      AlignmentControl = uniAlignmentClient
      Align = alClient
      Anchors = [akLeft, akTop, akRight, akBottom]
      TabOrder = 0
      ExplicitLeft = 1
      ExplicitTop = 30
      ExplicitWidth = 695
      ExplicitHeight = 351
      inherited PageControl: TUniPageControl
        Width = 695
        Height = 322
        ExplicitWidth = 695
        ExplicitHeight = 322
      end
      inherited TopToolBar: TUniToolBar
        Width = 695
      end
    end
    object Panel2: TUniPanel
      Left = 1
      Top = 1
      Width = 695
      Height = 29
      Hint = ''
      Align = alTop
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 1
      Caption = ''
      object sbtnBack: TUniSpeedButton
        Left = 38
        Top = 1
        Width = 26
        Height = 26
        Hint = 'Move Back'
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        TabOrder = 2
        OnClick = sbtnBackClick
      end
      object sbtnForward: TUniSpeedButton
        Left = 64
        Top = 1
        Width = 26
        Height = 26
        Hint = 'Move Forward'
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        TabOrder = 3
        OnClick = sbtnForwardClick
      end
      object bbtnSearch: TUniSpeedButton
        Left = 1
        Top = 1
        Width = 26
        Height = 26
        Hint = 'Search...'
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        TabOrder = 4
        OnClick = bbtnSearchClick
      end
      object Bevel1: TUniPanel
        Left = 32
        Top = 1
        Width = 2
        Height = 26
        Hint = ''
        TabOrder = 5
        Caption = ''
      end
      object Panel3: TUniPanel
        Left = 638
        Top = 1
        Width = 57
        Height = 27
        Hint = ''
        ShowHint = True
        ParentShowHint = False
        Align = alRight
        Anchors = [akTop, akRight, akBottom]
        TabOrder = 0
        Caption = ''
        object Shape1: TUniPanel
          Left = 2
          Top = 8
          Width = 10
          Height = 11
          Hint = 'Available'
          ShowHint = True
          TabOrder = 1
          Caption = ''
        end
        object Shape4: TUniPanel
          Left = 29
          Top = 8
          Width = 10
          Height = 11
          Hint = 'Over-booked'
          ShowHint = True
          TabOrder = 2
          Caption = ''
        end
        object Shape3: TUniPanel
          Left = 15
          Top = 8
          Width = 10
          Height = 11
          Hint = 'Booked'
          ShowHint = True
          TabOrder = 3
          Caption = ''
        end
      end
    end
  end
  object pnlCriteria: TUniPanel
    Left = 0
    Top = 0
    Width = 181
    Height = 382
    Hint = ''
    Align = alLeft
    Anchors = [akLeft, akTop, akBottom]
    TabOrder = 1
    Caption = ''
    object Label1: TUniLabel
      Left = 4
      Top = 38
      Width = 52
      Height = 13
      Hint = ''
      Caption = 'Begin Date'
      TabOrder = 4
    end
    object Label2: TUniLabel
      Left = 4
      Top = 66
      Width = 55
      Height = 13
      Hint = ''
      Caption = 'View Period'
      TabOrder = 5
    end
    object Panel4: TUniPanel
      Left = 1
      Top = 1
      Width = 179
      Height = 29
      Hint = ''
      Align = alTop
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 0
      Caption = ''
      object sbtnApply: TUniSpeedButton
        Left = 1
        Top = 1
        Width = 26
        Height = 26
        Hint = 'Apply Criteria'
        ShowHint = True
        ParentShowHint = False
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        Caption = ''
        ParentColor = False
        Color = clWindow
        TabOrder = 1
        OnClick = sbtnApplyClick
      end
    end
    object wwDBDateTimePickerStartDate: TUniDBDateTimePicker
      Left = 69
      Top = 34
      Width = 106
      Height = 21
      Hint = ''
      DateTime = 42656.000000000000000000
      DateFormat = 'dd/MM/yyyy'
      TimeFormat = 'HH:mm:ss'
      TabOrder = 1
    end
    object wwDBComboBoxViewType: TUniDBComboBox
      Left = 69
      Top = 62
      Width = 106
      Height = 21
      Hint = ''
      Style = csDropDownList
      Items.Strings = (
        'Monthly M'
        'Weekly W'
        'Daily D')
      TabOrder = 2
      OnChange = wwDBComboBoxViewTypeChange
    end
  end
  object SrcCapacityPlan: TDataSource
    DataSet = QryMonthly
    Left = 245
    Top = 97
  end
  object QryMonthly: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      ''
      '       -- period 1'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      '       a.hrs_week_avail * 4 as hrs_avail_1,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                    '
      '       -- period 2'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      '       a.hrs_week_avail * 4 hrs_avail_2,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                              '
      '       -- period 3'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      '       a.hrs_week_avail * 4 hrs_avail_3,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      '       a.hrs_week_avail * 4 hrs_avail_4,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      '       a.hrs_week_avail * 4 hrs_avail_5,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    -1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    -1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      '       v_mfgtype_emp_level v,'
      '       v_day_labor_week r,'
      '       v_labor_week_avail a'
      ' where'
      '       v.emp_level_id = a.emp_level_id(+)'
      '   and rtrim(v.mfgtype) = rtrim(a.mfgtype(+))'
      '   and nvl(v.mfgcell_id,0) = NVL(a.mfgcell_id(+),0)'
      '   and v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      '   and NVL(v.mfgcell_id, 0) = NVL( r.mfgcell_id(+), 0)'
      ''
      '   and misc.eplant_filter( v.eplant_id ) = 1'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.mfgcell_id,'
      '       v.mfgcell,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       a.hrs_week_avail'
      ''
      '-- HPICK_PROPAGATE_SORT_START'
      '-- do not remove these comments'
      'order by'
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.emp_level_code'
      '-- HPICK_PROPAGATE_SORT_END'
      ''
      '')
    Left = 244
    Top = 134
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDateTime
        ParamType = ptInput
      end>
    object QryMonthlyEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryMonthlyPeriod1: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period1'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod2: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period2'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod3: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period3'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod4: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period4'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyPeriod5: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
    object QryMonthlyHRS_REQ_FUTURE: TFMTBCDField
      DisplayLabel = 'Future Hours'
      DisplayWidth = 17
      FieldName = 'HRS_REQ_FUTURE'
      ReadOnly = True
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyMFGTYPE: TStringField
      DisplayLabel = 'Mfg Type'
      DisplayWidth = 9
      FieldName = 'MFGTYPE'
      ReadOnly = True
      Visible = False
      FixedChar = True
      Size = 10
    end
    object QryMonthlyMFGCELL: TStringField
      DisplayLabel = 'Mfg Cell'
      DisplayWidth = 9
      FieldName = 'MFGCELL'
      ReadOnly = True
      Visible = False
      FixedChar = True
      Size = 15
    end
    object QryMonthlyHRS_REQ_1: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_1'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyHRS_AVAIL_1: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_1'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyHRS_REQ_2: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_2'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyHRS_AVAIL_2: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_2'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyHRS_REQ_3: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_3'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyHRS_AVAIL_3: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_3'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyHRS_REQ_4: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_4'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      Size = 38
    end
    object QryMonthlyHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      Size = 38
    end
    object QryMonthlyMFGCELL_ID: TBCDField
      DisplayWidth = 10
      FieldName = 'MFGCELL_ID'
      ReadOnly = True
      Visible = False
      Size = 0
    end
    object QryMonthlyHRS_AVAIL_4: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_4'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Visible = False
      Size = 0
    end
    object QryMonthlySETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlySETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlySETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlySETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlySETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlySETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
      Size = 38
    end
  end
  object MainMenu1: TUniMainMenu
    Left = 324
    Top = 97
    object File1: TUniMenuItem
      Caption = 'File'
      object QueryByMfgTypeMfgCellEmployeeLevel1: TUniMenuItem
        Caption = 'Query By Mfg Type + Mfg Cell + Employee Level'
        Checked = True
        RadioItem = True
        OnClick = QueryByMfgTypeMfgCellEmployeeLevel1Click
      end
      object QueryByMfgTypeEmployeeLevel1: TUniMenuItem
        Caption = 'Query By Mfg Type + Employee Level'
        RadioItem = True
        OnClick = QueryByMfgTypeEmployeeLevel1Click
      end
      object N1: TUniMenuItem
        Caption = '-'
      end
      object Close1: TUniMenuItem
        Caption = 'Exit'
        OnClick = Close1Click
      end
    end
    object Help1: TUniMenuItem
      Caption = 'Help'
      object Contents1: TUniMenuItem
        Caption = 'Contents'
        OnClick = Contents1Click
      end
      object About1: TUniMenuItem
        Caption = 'About'
        OnClick = About1Click
      end
    end
  end
  object QryWeekly: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      ''
      '       -- period 1'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      '       a.hrs_week_avail as hrs_avail_1,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                  '
      '       -- period 2'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      '       a.hrs_week_avail as hrs_avail_2,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                                  '
      '       -- period 3'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      '       a.hrs_week_avail as hrs_avail_3,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      '       a.hrs_week_avail as hrs_avail_4,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      '       a.hrs_week_avail as hrs_avail_5,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    -1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    -1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      '       v_mfgtype_emp_level v,'
      '       v_day_labor_week r,'
      '       v_labor_week_avail a'
      ' where'
      '       v.emp_level_id = a.emp_level_id(+)'
      '   and rtrim(v.mfgtype) = rtrim(a.mfgtype(+))'
      '   and nvl(v.mfgcell_id,0) = NVL(a.mfgcell_id(+),0)'
      '   '
      '   and v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      '   and NVL(v.mfgcell_id, 0) = NVL( r.mfgcell_id(+), 0)'
      ''
      '   and misc.eplant_filter( v.eplant_id ) = 1'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.mfgcell_id,'
      '       v.mfgcell,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       a.hrs_week_avail'
      ''
      '-- HPICK_PROPAGATE_SORT_START'
      '-- do not remove these comments'
      'order by'
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.emp_level_code'
      '-- HPICK_PROPAGATE_SORT_END'
      ''
      '')
    Left = 276
    Top = 134
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDateTime
        ParamType = ptInput
      end>
    object QryWeeklyEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryWeeklyMFGTYPE: TStringField
      FieldName = 'MFGTYPE'
      FixedChar = True
      Size = 10
    end
    object QryWeeklyMFGCELL: TStringField
      FieldName = 'MFGCELL'
      FixedChar = True
      Size = 15
    end
    object QryWeeklyMFGCELL_ID: TBCDField
      FieldName = 'MFGCELL_ID'
      Size = 0
    end
    object QryWeeklyHRS_REQ_1: TFMTBCDField
      FieldName = 'HRS_REQ_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_1: TFMTBCDField
      FieldName = 'HRS_AVAIL_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_REQ_2: TFMTBCDField
      FieldName = 'HRS_REQ_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_2: TFMTBCDField
      FieldName = 'HRS_AVAIL_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_REQ_3: TFMTBCDField
      FieldName = 'HRS_REQ_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_3: TFMTBCDField
      FieldName = 'HRS_AVAIL_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_REQ_4: TFMTBCDField
      FieldName = 'HRS_REQ_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_4: TFMTBCDField
      FieldName = 'HRS_AVAIL_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyHRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyPeriod1: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period1'
      Calculated = True
    end
    object QryWeeklyPeriod2: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period2'
      Calculated = True
    end
    object QryWeeklyPeriod3: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period3'
      Calculated = True
    end
    object QryWeeklyPeriod4: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period4'
      Calculated = True
    end
    object QryWeeklyPeriod5: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
    object QryWeeklyEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Visible = False
      Size = 0
    end
    object QryWeeklySETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklySETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
      Size = 38
    end
  end
  object QryDaily: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       '
      '       -- period 1'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    1, a.hrs_avail,'
      '                    0)) as hrs_avail_1,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                                   '
      '       -- period 2'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    2, a.hrs_avail,'
      '                    0)) as hrs_avail_2,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                                  '
      '       -- period 3'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    3, a.hrs_avail,'
      '                    0)) as hrs_avail_3,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    4, a.hrs_avail,'
      '                    0)) as hrs_avail_4,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    5, a.hrs_avail,'
      '                    0)) as hrs_avail_5,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( Sign( r.prod_date - trunc(:start_date) + 1 -' +
        ' 6),'
      '                    +1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( Sign( r.prod_date - trunc(:start_date) + 1 -' +
        ' 6),'
      '                    +1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      '       v_mfgtype_emp_level v,'
      '       v_day_labor_day r,'
      '       v_labor_day_avail a'
      ' where'
      '       v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      '   and NVL(v.mfgcell_id, 0) = NVL( r.mfgcell_id(+), 0)'
      ''
      '   and r.emp_level_id = a.emp_level_id(+)'
      '   and rtrim(r.mfgtype) = rtrim(a.mfgtype(+))'
      '   and nvl(r.mfgcell_id,0) = NVL(a.mfgcell_id(+),0)'
      '   '
      
        '   /* and trunc(r.prod_date) = trunc( __start_date) + a.week_day' +
        '(+) - 1 */'
      '   and IQCalendar.day_of_week(r.prod_date) = a.week_day(+) '
      '   '
      '   and misc.eplant_filter( v.eplant_id ) = 1'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.mfgcell_id,'
      '       v.mfgcell,'
      '       v.emp_level_id,'
      '       v.emp_level_code'
      '       '
      '-- HPICK_PROPAGATE_SORT_START'
      '-- do not remove these comments'
      'order by'
      '       v.mfgtype,'
      '       v.mfgcell,'
      '       v.emp_level_code'
      '-- HPICK_PROPAGATE_SORT_END'
      ''
      '')
    Left = 308
    Top = 134
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDateTime
        ParamType = ptInput
      end>
    object QryDailyEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryDailyMFGTYPE: TStringField
      FieldName = 'MFGTYPE'
      FixedChar = True
      Size = 10
    end
    object QryDailyMFGCELL: TStringField
      FieldName = 'MFGCELL'
      FixedChar = True
      Size = 15
    end
    object QryDailyMFGCELL_ID: TBCDField
      FieldName = 'MFGCELL_ID'
      Size = 0
    end
    object QryDailyHRS_REQ_1: TFMTBCDField
      FieldName = 'HRS_REQ_1'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_AVAIL_1: TFMTBCDField
      FieldName = 'HRS_AVAIL_1'
      DisplayFormat = '########0.00'
    end
    object QryDailyPeriod1: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period1'
      Calculated = True
    end
    object QryDailyPeriod2: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period2'
      Calculated = True
    end
    object QryDailyPeriod3: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period3'
      Calculated = True
    end
    object QryDailyPeriod4: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period4'
      Calculated = True
    end
    object QryDailyPeriod5: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
    object QryDailyHRS_REQ_2: TFMTBCDField
      FieldName = 'HRS_REQ_2'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_AVAIL_2: TFMTBCDField
      FieldName = 'HRS_AVAIL_2'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_REQ_3: TFMTBCDField
      FieldName = 'HRS_REQ_3'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_AVAIL_3: TFMTBCDField
      FieldName = 'HRS_AVAIL_3'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_REQ_4: TFMTBCDField
      FieldName = 'HRS_REQ_4'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_AVAIL_4: TFMTBCDField
      FieldName = 'HRS_AVAIL_4'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      DisplayFormat = '########0.00'
    end
    object QryDailyHRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
    end
    object QryDailyEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Visible = False
      Size = 0
    end
    object QryDailySETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      DisplayFormat = '########0.00'
    end
    object QryDailySETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      DisplayFormat = '########0.00'
    end
    object QryDailySETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      DisplayFormat = '########0.00'
    end
    object QryDailySETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      DisplayFormat = '########0.00'
    end
    object QryDailySETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      DisplayFormat = '########0.00'
    end
    object QryDailySETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
    end
  end
  object QryMonthlyEx: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       '#39'*'#39' as mfgcell,'
      '       0 as mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      ''
      '       -- period 1'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      '       a.hrs_week_avail * 4 as hrs_avail_1,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                    '
      '       -- period 2'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      '       a.hrs_week_avail * 4 hrs_avail_2,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                              '
      '       -- period 3'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      '       a.hrs_week_avail * 4 hrs_avail_3,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      '       a.hrs_week_avail * 4 hrs_avail_4,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      '       a.hrs_week_avail * 4 hrs_avail_5,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    -1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start, 4 ),'
      '                    -1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      '       v_mfgtype_emp_level_ex v,'
      '       v_day_labor_week_ex r,'
      '       v_labor_week_avail_ex a'
      ' where'
      '       v.emp_level_id = a.emp_level_id(+)'
      '   and rtrim(v.mfgtype) = rtrim(a.mfgtype(+))'
      ''
      '   and v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      ''
      '   and misc.eplant_filter( v.eplant_id ) = 1'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       a.hrs_week_avail'
      ''
      '-- HPICK_PROPAGATE_SORT_START'
      '-- do not remove these comments'
      'order by'
      '       v.mfgtype,'
      '       v.emp_level_code'
      '-- HPICK_PROPAGATE_SORT_END'
      ''
      '')
    Left = 244
    Top = 166
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDateTime
        ParamType = ptInput
      end>
    object QryMonthlyExEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryMonthlyExPeriod1: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period1'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyExPeriod2: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period2'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyExPeriod3: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period3'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyExPeriod4: TFMTBCDField
      DisplayWidth = 27
      FieldKind = fkCalculated
      FieldName = 'Period4'
      ReadOnly = True
      Calculated = True
    end
    object QryMonthlyExPeriod5: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
    object QryMonthlyExHRS_REQ_FUTURE: TFMTBCDField
      DisplayLabel = 'Future Hours'
      DisplayWidth = 17
      FieldName = 'HRS_REQ_FUTURE'
      ReadOnly = True
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExMFGTYPE: TStringField
      DisplayLabel = 'Mfg Type'
      DisplayWidth = 9
      FieldName = 'MFGTYPE'
      ReadOnly = True
      Visible = False
      FixedChar = True
      Size = 10
    end
    object QryMonthlyExMFGCELL: TStringField
      DisplayLabel = 'Mfg Cell'
      DisplayWidth = 9
      FieldName = 'MFGCELL'
      ReadOnly = True
      Visible = False
      FixedChar = True
      Size = 1
    end
    object QryMonthlyExHRS_REQ_1: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_1'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExHRS_AVAIL_1: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_1'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExHRS_REQ_2: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_2'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExHRS_AVAIL_2: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_2'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExHRS_REQ_3: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_3'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExHRS_AVAIL_3: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_3'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExHRS_REQ_4: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'HRS_REQ_4'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      Size = 38
    end
    object QryMonthlyExHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      Size = 38
    end
    object QryMonthlyExMFGCELL_ID: TFMTBCDField
      DisplayWidth = 10
      FieldName = 'MFGCELL_ID'
      ReadOnly = True
      Visible = False
      Size = 0
    end
    object QryMonthlyExHRS_AVAIL_4: TFMTBCDField
      DisplayWidth = 11
      FieldName = 'HRS_AVAIL_4'
      ReadOnly = True
      Visible = False
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Visible = False
      Size = 0
    end
    object QryMonthlyExSETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExSETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExSETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExSETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExSETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryMonthlyExSETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
      Size = 38
    end
  end
  object QryWeeklyEx: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       '#39'*'#39' as mfgcell,'
      '       0 as mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      ''
      '       -- period 1'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      '       a.hrs_week_avail as hrs_avail_1,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                  '
      '       -- period 2'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      '       a.hrs_week_avail as hrs_avail_2,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                                  '
      '       -- period 3'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      '       a.hrs_week_avail as hrs_avail_3,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      '       a.hrs_week_avail as hrs_avail_4,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      '       a.hrs_week_avail as hrs_avail_5,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    -1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( iqcalendar.belongs_to_week_period( :start_da' +
        'te, r.week_start ),'
      '                    -1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      '       v_mfgtype_emp_level_ex v,'
      '       v_day_labor_week_ex r,'
      '       v_labor_week_avail_ex a'
      ' where'
      '       v.emp_level_id = a.emp_level_id(+)'
      '   and rtrim(v.mfgtype) = rtrim(a.mfgtype(+))'
      '   '
      '   and v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      ''
      '   and misc.eplant_filter( v.eplant_id ) = 1'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       a.hrs_week_avail'
      ''
      '-- HPICK_PROPAGATE_SORT_START'
      '-- do not remove these comments'
      'order by'
      '       v.mfgtype,'
      '       v.emp_level_code'
      '-- HPICK_PROPAGATE_SORT_END'
      ''
      '')
    Left = 276
    Top = 166
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDateTime
        ParamType = ptInput
      end>
    object QryWeeklyExEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryWeeklyExMFGTYPE: TStringField
      FieldName = 'MFGTYPE'
      FixedChar = True
      Size = 10
    end
    object QryWeeklyExMFGCELL: TStringField
      FieldName = 'MFGCELL'
      FixedChar = True
      Size = 1
    end
    object QryWeeklyExMFGCELL_ID: TFMTBCDField
      FieldName = 'MFGCELL_ID'
      Size = 0
    end
    object QryWeeklyExHRS_REQ_1: TFMTBCDField
      FieldName = 'HRS_REQ_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_AVAIL_1: TFMTBCDField
      FieldName = 'HRS_AVAIL_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_REQ_2: TFMTBCDField
      FieldName = 'HRS_REQ_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_AVAIL_2: TFMTBCDField
      FieldName = 'HRS_AVAIL_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_REQ_3: TFMTBCDField
      FieldName = 'HRS_REQ_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_AVAIL_3: TFMTBCDField
      FieldName = 'HRS_AVAIL_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_REQ_4: TFMTBCDField
      FieldName = 'HRS_REQ_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_AVAIL_4: TFMTBCDField
      FieldName = 'HRS_AVAIL_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExHRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExPeriod1: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period1'
      Calculated = True
    end
    object QryWeeklyExPeriod2: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period2'
      Calculated = True
    end
    object QryWeeklyExPeriod3: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period3'
      Calculated = True
    end
    object QryWeeklyExPeriod4: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period4'
      Calculated = True
    end
    object QryWeeklyExPeriod5: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
    object QryWeeklyExEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Visible = False
      Size = 0
    end
    object QryWeeklyExSETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExSETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExSETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExSETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExSETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      DisplayFormat = '########0.00'
      Size = 38
    end
    object QryWeeklyExSETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
      Size = 38
    end
  end
  object QryDailyEx: TFDQuery
    Tag = 1
    BeforeOpen = QryMonthlyBeforeOpen
    BeforeInsert = AbortOnInsert
    CachedUpdates = True
    OnUpdateRecord = QryMonthlyUpdateRecord
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      'select '
      '       v.mfgtype,'
      '       '#39'*'#39' as mfgcell,'
      '       0 as mfgcell_id,'
      '       v.emp_level_id,'
      '       v.emp_level_code,'
      '       '
      '       -- period 1'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    1, r.labor_hrs,'
      '                    0 )) as hrs_req_1,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    1, a.hrs_avail,'
      '                    0)) as hrs_avail_1,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_1,'
      '                                   '
      '       -- period 2'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    2, r.labor_hrs,'
      '                    0 )) as hrs_req_2,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    2, a.hrs_avail,'
      '                    0)) as hrs_avail_2,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    2, r.setuphrs,'
      '                    0 )) as setup_hrs_req_2,'
      '                                  '
      '       -- period 3'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    3, r.labor_hrs,'
      '                    0 )) as hrs_req_3,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    3, a.hrs_avail,'
      '                    0)) as hrs_avail_3,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    3, r.setuphrs,'
      '                    0 )) as setup_hrs_req_3,'
      '                                           '
      '       -- period 4'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    4, r.labor_hrs,'
      '                    0 )) as hrs_req_4,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    4, a.hrs_avail,'
      '                    0)) as hrs_avail_4,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    4, r.setuphrs,'
      '                    0 )) as setup_hrs_req_4,'
      '                                           '
      '       -- period 5'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    5, r.labor_hrs,'
      '                    0 )) as hrs_req_5,'
      
        '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,' +
        '       '
      '                    5, a.hrs_avail,'
      '                    0)) as hrs_avail_5,'
      '       sum( decode( trunc(r.prod_date) - trunc(:start_date) + 1,'
      '                    5, r.setuphrs,'
      '                    0 )) as setup_hrs_req_5,'
      '                                           '
      '       -- outside'
      
        '       sum( decode( Sign( r.prod_date - trunc(:start_date) + 1 -' +
        ' 6),'
      '                    +1, r.labor_hrs,'
      '                    0 )) as hrs_req_future,'
      
        '       sum( decode( Sign( r.prod_date - trunc(:start_date) + 1 -' +
        ' 6),'
      '                    +1, r.setuphrs,'
      '                    0 )) as setup_hrs_req_future'
      '                                           '
      '  from '
      '       v_mfgtype_emp_level_ex v,'
      '       v_day_labor_day_ex r,'
      '       v_labor_day_avail_ex a'
      ' where'
      '       v.emp_level_id  = r.emp_level_id(+)'
      '   and v.mfgtype = r.mfgtype(+)'
      ''
      '   and r.emp_level_id = a.emp_level_id(+)'
      '   and rtrim(r.mfgtype) = rtrim(a.mfgtype(+))'
      '   '
      
        '   /* and trunc(r.prod_date) = trunc(__start_date) + a.week_day(' +
        '+) - 1 */'
      '   and IQCalendar.day_of_week(r.prod_date) = a.week_day(+) '
      '                  '
      '   and misc.eplant_filter( v.eplant_id ) = 1'
      ''
      'group by '
      '       v.mfgtype,'
      '       v.emp_level_id,'
      '       v.emp_level_code'
      '       '
      '-- HPICK_PROPAGATE_SORT_START'
      '-- do not remove these comments'
      'order by'
      '       v.mfgtype,'
      '       v.emp_level_code'
      '-- HPICK_PROPAGATE_SORT_END'
      ''
      '')
    Left = 308
    Top = 166
    ParamData = <
      item
        Name = 'start_date'
        DataType = ftDateTime
        ParamType = ptInput
      end>
    object QryDailyExEMP_LEVEL_CODE: TStringField
      FieldName = 'EMP_LEVEL_CODE'
      Size = 10
    end
    object QryDailyExMFGTYPE: TStringField
      FieldName = 'MFGTYPE'
      FixedChar = True
      Size = 10
    end
    object QryDailyExMFGCELL: TStringField
      FieldName = 'MFGCELL'
      FixedChar = True
      Size = 1
    end
    object QryDailyExMFGCELL_ID: TFMTBCDField
      FieldName = 'MFGCELL_ID'
      Size = 0
    end
    object QryDailyExHRS_REQ_1: TFMTBCDField
      FieldName = 'HRS_REQ_1'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_AVAIL_1: TFMTBCDField
      FieldName = 'HRS_AVAIL_1'
      DisplayFormat = '########0.00'
    end
    object QryDailyExPeriod1: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period1'
      Calculated = True
    end
    object QryDailyExPeriod2: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period2'
      Calculated = True
    end
    object QryDailyExPeriod3: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period3'
      Calculated = True
    end
    object QryDailyExPeriod4: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period4'
      Calculated = True
    end
    object QryDailyExPeriod5: TFMTBCDField
      FieldKind = fkCalculated
      FieldName = 'Period5'
      Calculated = True
    end
    object QryDailyExHRS_REQ_2: TFMTBCDField
      FieldName = 'HRS_REQ_2'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_AVAIL_2: TFMTBCDField
      FieldName = 'HRS_AVAIL_2'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_REQ_3: TFMTBCDField
      FieldName = 'HRS_REQ_3'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_AVAIL_3: TFMTBCDField
      FieldName = 'HRS_AVAIL_3'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_REQ_4: TFMTBCDField
      FieldName = 'HRS_REQ_4'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_AVAIL_4: TFMTBCDField
      FieldName = 'HRS_AVAIL_4'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_REQ_5: TFMTBCDField
      FieldName = 'HRS_REQ_5'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_AVAIL_5: TFMTBCDField
      FieldName = 'HRS_AVAIL_5'
      DisplayFormat = '########0.00'
    end
    object QryDailyExHRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
    end
    object QryDailyExEMP_LEVEL_ID: TBCDField
      FieldName = 'EMP_LEVEL_ID'
      Visible = False
      Size = 0
    end
    object QryDailyExSETUP_HRS_REQ_1: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_1'
      DisplayFormat = '########0.00'
    end
    object QryDailyExSETUP_HRS_REQ_2: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_2'
      DisplayFormat = '########0.00'
    end
    object QryDailyExSETUP_HRS_REQ_3: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_3'
      DisplayFormat = '########0.00'
    end
    object QryDailyExSETUP_HRS_REQ_4: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_4'
      DisplayFormat = '########0.00'
    end
    object QryDailyExSETUP_HRS_REQ_5: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_5'
      DisplayFormat = '########0.00'
    end
    object QryDailyExSETUP_HRS_REQ_FUTURE: TFMTBCDField
      FieldName = 'SETUP_HRS_REQ_FUTURE'
      DisplayFormat = '########0.00'
    end
  end
end
